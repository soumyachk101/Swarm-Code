...
/// providers that load a file Droppy Code installs: Pi (an extension) and
...
/// from the file at `DROPPY_CODE_MCP_CONFIG`
...
enum MCPBridge {
    ...
    function connectStdio(server, entry) {
      return new Promise((resolve, reject) => {
        ...
        (async () => {
          await request("initialize", { protocolVersion: PROTOCOL_VERSION, capabilities: {}, clientInfo: { name: "Droppy Code" } });
    ...
    export async function loadMCPTools() {
      ...
      for (const server of Object.keys(servers)) {
        ...
        try {
          ...
          console.error("droppy mcp: skipping " + server + ": " + errorText(error));
        }
      }
      return tools;
    }
    ...
    static func installBridge() throws -> URL {
        ...
        let file = directory.appendingPathComponent("droppy-mcp-bridge.mjs")
