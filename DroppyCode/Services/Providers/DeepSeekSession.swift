...
@MainActor
final class DeepSeekSession: ProviderSession {
    ...
    private func performStream(payload: [String: JSONValue]) async throws -> StreamRound {
        ...
        let stream = AsyncStream<StreamChunk> { continuation in
            ...
            let drain = Task.detached(priority: .userInitiated) {
                do {
                    try await Self.drainSSE(bytes: bytes, into: coalescer, contextWindow: DeepSeekAPI.contextWindow)
                } catch is CancellationError {
    ...
    private static func errorMessage(fromBody body: String, statusCode: Int) -> String {
        ...
        return "DeepSeek returned status \(statusCode)."
    }
    ...
    private func systemPrompt() -> String {
        ...
        return """
        You are Droppy Code's DeepSeek coding agent, working inside \(workingDirectory) on macOS.
        \(modeLine)
        Rules:
