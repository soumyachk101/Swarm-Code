...
///
///     open -n -W "Droppy Code.app" --args --website-captures <folder>
///
...
enum WebsiteCaptures {
    /// The run's folder, flags, storage and defaults live in `CaptureRun` (Core), where the
    /// store and the providers can ask for them; these read through to it.
    nonisolated static var outputDirectory: URL? { CaptureRun.outputDirectory }
    nonisolated static var isTourRun: Bool { CaptureRun.isTourRun }
    nonisolated static var isEnabled: Bool { CaptureRun.isEnabled }
    nonisolated static var storageRoot: URL? { CaptureRun.storageRoot }
    nonisolated static var defaults: UserDefaults? { CaptureRun.defaults }
    ...
    static func prepare() {
        ...
        defaults.removePersistentDomain(forName: CaptureRun.suiteName)
        ...
        var library = Library()
        library.projects = [
            project(ID.droppyCode, "Droppy Code", repos, ["DroppyCode/App/Views/Composer/ComposerView.swift": composerSource]),
    ...
    private static func composerHistory(_ now: Date) -> ThreadDocument {
        ...
        var read = ToolCall(kind: .read, title: "Read", detail: "DroppyCode/App/Views/Composer/ComposerView.swift")
    ...
    diff --git a/DroppyCode/App/Views/Composer/ComposerView.swift b/DroppyCode/App/Views/Composer/ComposerView.swift
    --- a/DroppyCode/App/Views/Composer/ComposerView.swift
    +++ b/DroppyCode/App/Views/Composer/ComposerView.swift
    ...
    static func editingScene(_ model: AppModel, _ stage: Stage, _ recorder: Recorder, film: Bool = true) async {
        ...
        runtime.rehearseTurn(
            ...
            touchedPaths: ["DroppyCode/App/Views/Composer/ComposerView.swift"],
        ...
        runtime.rehearse(.toolStarted(id: "t-read", call: ToolCall(kind: .read, title: "Read", detail: "DroppyCode/App/Views/Composer/ComposerView.swift")))
        ...
        runtime.rehearse(.toolUpdated(id: "t-edit", update: ToolUpdate(
            status: .completed,
            edits: [FileEdit(path: "DroppyCode/App/Views/Composer/ComposerView.swift", diff: composerPatch, additions: 3, deletions: 1)]
        )))
        ...
        runtime.rehearse(.toolStarted(id: "t-build", call: ToolCall(kind: .command, title: "xcodebuild -scheme DroppyCode build", detail: nil)))
    ...
    static func questionScene(_ model: AppModel, _ stage: Stage, _ recorder: Recorder, film: Bool = true, stillName: String? = nil) async {
        ...
        runtime.rehearse(.toolStarted(id: "t-tl", call: ToolCall(kind: .read, title: "Read", detail: "DroppyCode/App/Views/Chat/TimelineRows.swift")))
        try? await Task.sleep(for: .milliseconds(500))
        ...
        try? await Task.sleep(for: .milliseconds(700))
        runtime.rehearse(.toolUpdated(id: "t-edit2", update: ToolUpdate(status: .completed, edits: [FileEdit(path: "DroppyCode/App/Views/Chat/TimelineRows.swift", additions: 18, deletions: 3)])))
        try? await Task.sleep(for: .milliseconds(500))
