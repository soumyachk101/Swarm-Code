...
enum PiCLI {
    ...
    export default async function (pi) {
      try {
        ...
        console.error("swarm mcp: " + (error instanceof Error ? error.message : String(error)));
      }
      ...
      pi.on("tool_call", async (event, ctx) => {
        ...
        const answer = await ctx.ui.select("swarm-gate " + JSON.stringify(payload), ["Allow", "Allow for session", "Deny"]);
