...
final class CopilotSession: ProviderSession {
    ...
    private func sessionParameters() -> [String: JSONValue] {
        ...
        if let hydra = configuration.hydra {
            if hydra.runsNatively {
                ...
                // Heads on another provider are Swarm-run: the lead asks for them with the
    ...
    private static func connect(
        ...
        let pong = try await connection.request("ping", ["message": "swarm-code"])
    ...
    private func handleNotification(_ method: String, _ params: JSONValue) {
        ...
        case "assistant.usage":
            ...
            if spend > 0 {
                ...
                // One report per message, so it is already the amount that message
                // consumed. A head's report carries its id: it is the account's
                // spend, and the head's alone, not the lead's.
                onEvent?(.tokenSpend(TokenSpend(
                    provider: .copilot,
                    totalTokens: spend,
                    inputTokens: data["inputTokens"]?.int,
                    outputTokens: data["outputTokens"]?.int,
                    cachedInputTokens: data["cacheReadTokens"]?.int,
