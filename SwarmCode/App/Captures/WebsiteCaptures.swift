...
enum WebsiteCaptures {
    ...
    private static func project(_ id: UUID, _ name: String, _ repos: URL, _ files: [String: String]) -> Project {
        ...
        git.arguments = ["-c", "git init -q -b main && git -c user.name=Swarm -c user.email=hi@getswarm.app add -A && git -c user.name=Swarm -c user.email=hi@getswarm.app commit -q -m 'Initial import' >/dev/null 2>&1"]
