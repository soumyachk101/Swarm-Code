...
struct TokenActivitySection: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            legend(total: ledger.dailyTotals.values.reduce(0, +), uncertainTokens: ledger.uncertainTokensThisYear)
        }
    ...
    /// `uncertainTokens` is the part of the total recorded before the ledger kept where its
    /// tokens came from: it stays in the total, and naming it is better than presenting it
    /// as measured.
    private func legend(total: Int, uncertainTokens: Int) -> some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            if uncertainTokens > 0 {
                Text(verbatim: "\(UsagePanel.tokens(uncertainTokens)) of these were recorded before spend kept its source, so they may overlap imported history.")
                    .font(.system(size: 11))
                    .foregroundStyle(Chrome.secondaryText)
        ...
        .accessibilityValue(Text(uncertainTokens > 0
            ? "\(UsagePanel.tokens(total)) tokens recorded, \(UsagePanel.tokens(uncertainTokens)) of them without a kept source"
            : "\(UsagePanel.tokens(total)) tokens recorded"))
    }
