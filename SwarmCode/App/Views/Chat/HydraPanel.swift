...
/// Swarm-run head keeps its own chat box at the bottom, so it can be steered or answered;
            ...
            HStack(spacing: 8) {
                if isPoppedOut {
                    ...
                    if let selected, let popOut, heads.count > 1 {
                        ChromeCircleButton(symbol: "arrow.up.right", help: "Pop \(selected.hydra?.displayName ?? "this head") out into its own panel") {
                            popOut(selected.id)
                        }
                        .transition(.softAppear)
                    }
...
private struct HydraHeadRow: View {
    ...
    var body: some View {
        if let stored = head.hydra {
            // A running head's note and counts live on its runtime, off the thread
            ...
            HStack(spacing: 10) {
                ...
                if info.status == .running, info.canStop {
                    HydraStopButton(name: info.displayName) {
            ...
            .accessibilityLabel(Text("\(info.displayName), \(HydraStatusText.long(info)), \(info.task)\(isOnStage ? ", on stage" : "")"))
        }
    }
}
...
enum HydraStatusText {
    ...
    /// Where a Swarm-run head's work went, in a few words; nil for a head with no copy
    /// of its own.
