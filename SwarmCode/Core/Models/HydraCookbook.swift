...
enum HydraCookbook {
    ...
    static let recipes: [HydraPairRecipe] = [
        HydraPairRecipe(id: "claude-fable-opus", title: "Fable 5.1 leads Opus 5.5", tagline: "The deepest lead over the strongest builders: Fable plans and checks, Opus heads build. Set the thinking effort of each side yourself.", lead: .init(providers: [.claude, .commandcode], models: ["Fable"], effort: nil), heads: .init(providers: [.claude, .commandcode], models: ["Opus"], effort: nil), maxHeads: nil),
        HydraPairRecipe(id: "claude-opus-gemini-flash", title: "Opus 5.5 leads Gemini 3.8 Flash", tagline: "A careful lead and a wide, fast team: Opus writes the briefs, six Gemini Flash heads sprint through them.", lead: .init(providers: [.claude, .commandcode], models: ["Opus"], effort: "high"), heads: .init(providers: [.antigravity, .commandcode], models: ["Gemini 3.8 Flash", "Gemini", "Flash"], effort: "medium"), maxHeads: 6),
        HydraPairRecipe(id: "claude-opus-deepseek", title: "Opus 5.5 leads DeepSeek V4.1", tagline: "Opus keeps the plan tight; DeepSeek V4.1 Flash heads keep the bill small on the routine parts.", lead: .init(providers: [.claude, .commandcode], models: ["Opus"], effort: "high"), heads: .init(providers: [.deepseek, .commandcode], models: ["V4.1 Flash", "V4.1", "DeepSeek"], effort: "high"), maxHeads: nil),
        HydraPairRecipe(id: "claude-opus-glm", title: "Opus 5.5 leads GLM-5.3 Flash", tagline: "Opus keeps the plan tight; GLM-5.3 Flash heads on a Z.ai Coding Plan keep the bill flat.", lead: .init(providers: [.zai, .commandcode], models: ["5.3 Flash", "GLM"], effort: "high"), heads: .init(providers: [.zai, .commandcode], models: ["5.3 Flash", "GLM"], effort: "high"), maxHeads: nil),
        ...
        HydraPairRecipe(id: "astra-sol", title: "Astra 6 leads Sol 5.6", tagline: "Astra plans and reviews; Sol heads take the complex building at medium.", lead: .init(providers: [.codex, .commandcode], models: ["Astra"], effort: "high"), heads: .init(providers: [.codex, .commandcode], models: ["Sol"], effort: "medium"), maxHeads: nil),
        HydraPairRecipe(id: "sol-luna", title: "Sol 5.6 leads Luna 5.6", tagline: "Sol keeps the plan; quick Luna heads get through the routine parts.", lead: .init(providers: [.codex, .commandcode], models: ["Sol"], effort: "high"), heads: .init(providers: [.codex, .commandcode], models: ["Luna"], effort: "medium"), maxHeads: nil),
        ...
        HydraPairRecipe(id: "claude-opus-opus", title: "Opus 5.5 max, Opus 5.5 medium", tagline: "Opus thinks as hard as it can where it counts; Opus heads at medium do the building.", lead: .init(providers: [.claude, .commandcode], models: ["Opus"], effort: "max"), heads: .init(providers: [.claude, .commandcode], models: ["Opus"], effort: "medium"), maxHeads: nil),
