# Swarm Code installer (DMG) window styling

The Finder window users see when they open a Swarm Code DMG is styled by three
...
- Volume name must stay `Swarm Code` (the background image alias stores it).
- The background must be staged at exactly `/.background.tiff`.
- The app must be `Swarm Code.app` and the symlink `Applications` (their icon
...
1. Edit `generate_installer_background.swift` (all layout constants live at the
   ...
   ```bash
   swift release/dmg/generate_installer_background.swift /tmp/dmg-art "/Applications/Swarm Code.app/Contents/Resources/AppIcon.icns"
...
2. Only if geometry changed (window size, icon positions, icon size, or any
   ...
   any mounted `Swarm Code` volume before baking: otherwise the bake volume
   mounts as `Swarm Code 1` and the alias records that name (check with
   `base64 -d < release/dmg/installer-DS_Store.base64 | strings | grep Volumes`,
   which must print `/Volumes/Swarm Code`; `package_dmg.sh` refuses an asset
   that fails this check):

   ```bash
   python3 -m venv /tmp/dmgvenv && /tmp/dmgvenv/bin/pip install dmgbuild
   rm -rf /tmp/dmgbake && mkdir -p "/tmp/dmgbake/stub/Swarm Code.app/Contents/MacOS"
   ...
   /tmp/dmgvenv/bin/dmgbuild -s /tmp/dmgbake/dmgbuild_settings.py \
       -D here=/tmp/dmgbake "Swarm Code" /tmp/dmgbake/baked.dmg
...
3. Verify by packaging a build with `scripts/package_dmg.sh --app "/Applications/Swarm Code.app" --output /tmp/swarm-code.dmg`.
