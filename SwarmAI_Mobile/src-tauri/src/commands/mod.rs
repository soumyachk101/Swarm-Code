...
use super::bridge::MobileBridgeClient;
use super::models::{
    BridgeApproval, BridgeModels, BridgeStatus, BridgeThreadDetail, BridgeThreadSummary,
};
use super::store::{ConnectionStatus, MobileStore, SavedPairing};
...
pub async fn send_message(
    ...
    let uuid = Uuid::parse_str(&thread_id).map_err(|e| e.to_string())?;
    bridge
        .send_event(super::models::BridgeEvent::SendMessage {
            threadID: uuid,
            content,
        })
        .await
...
pub async fn approve_action(
    ...
    let uuid = Uuid::parse_str(&thread_id).map_err(|e| e.to_string())?;
    bridge
        .send_event(super::models::BridgeEvent::Approve {
            threadID: uuid,
            actionID: action_id,
        })
        .await
...
pub async fn reject_action(
    ...
    let uuid = Uuid::parse_str(&thread_id).map_err(|e| e.to_string())?;
    bridge
        .send_event(super::models::BridgeEvent::Reject {
            threadID: uuid,
            actionID: action_id,
        })
        .await
...
pub async fn create_thread(
    ...
    bridge
        .send_event(super::models::BridgeEvent::CreateThread { projectID: pid, prompt })
        .await
}
