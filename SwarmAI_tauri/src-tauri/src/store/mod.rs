...
pub struct AppStore {
 ...
 /// Currently configured providers (separate from the runtime registry, so
 /// users can edit them offline).
 pub providers: Vec<crate::models::provider::Provider>,
 /// Pinned library items (saved searches, drop-in snippets, etc.).
 pub library_items: Vec<LibraryItem>,
 // Convenience accessors for backwards compatibility with the Swift schema.
 pub projects: Vec<Project>,
 pub threads: Vec<ChatThread>,
 pub thread_documents: Vec<ThreadDocument>,
 pub hydra_pairs: Vec<HydraPair>,
