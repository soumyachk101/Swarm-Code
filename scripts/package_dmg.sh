...
[ -n "$APP_PATH" ] && [ -n "$OUTPUT_PATH" ] || error "Usage: --app <Swarm Code.app> --output <out.dmg>"
[ -d "$APP_PATH" ] || error "App bundle not found at $APP_PATH"

# The pre-baked .DS_Store keys icon positions and the background alias by
# name ("Swarm Code.app", volume "Swarm Code", "/.background.tiff"). Refuse anything
...
WORK_DIR="$(mktemp -d /tmp/swarm-code-dmg-XXXX)"
ACTIVE_MOUNT_POINT=""
cleanup() {
