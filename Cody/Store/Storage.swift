...
enum Storage {
    static let root: URL = {
        ...
        let url = base.appendingPathComponent("Cody", isDirectory: true)
    ...
    static var worktreesDirectory: URL {
        let url = URL(fileURLWithPath: LoginEnvironment.homeDirectory)
            .appendingPathComponent(".cody/worktrees", isDirectory: true)
        try? FileManager.default.createDirectory(at: url, withIntermediateDirectories: true)
        return url
    }
...
actor DiskWriter {
    ...
    func write(_ data: Data, to url: URL) {
        do {
            ...
            NSLog("Cody could not save %@: %@", url.lastPathComponent, error.localizedDescription)
        }
    }
}
