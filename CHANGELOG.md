...
All notable changes to Swarm Code are documented here, newest first. The format follows [Keep a Changelog](https://keepachangelog.com/en/1.1.0/); versions are the release tags without the `v`.
...
Swarm Code 1.5.6 stops a new chat from being told its agent had lost a conversation that never was, and draws the uncapped Heads at once choice as an infinity sign. Apple silicon, macOS 26 or later. Signed and notarized.
...
### Refinements
- The disk image opens to a styled drag-to-Applications window, with its own background and the app icon as the volume icon, the same installer window Swarm has.
...
Swarm Code 1.5.4 gets a Hydra team moving sooner: heads start the moment their brief is written rather than when the lead's whole reply is done, a lead can send heads to any project in the sidebar, and an older Claude Code no longer fails to launch when Hydra is on. Apple silicon, macOS 26 or later. Signed and notarized.
...
### Bug fixes
- With Hydra on, a Claude Code older than the flags Swarm Code passes it no longer fails at once with "unknown option": the app checks what the installed CLI understands and passes only that.
...
Swarm Code 1.5.3 puts the keyboard in charge: Escape stops a turn and takes it back, ⌘B hides the sidebar, ⌘1 to ⌘9 switch threads, a drag selects text across a whole reply, and a large Hydra team no longer freezes the app. Apple silicon, macOS 26 or later. Signed and notarized.
...
Swarm Code 1.5.2 fixes a crash the moment a lead's heads went out, and makes switching between chats with and without floating panels land in one step instead of gliding and jumping. Apple silicon, macOS 26 or later. Signed and notarized.
...
Swarm Code 1.5.0 fades streamed text in as it arrives, keeps a running turn's steps inside its working card, starts a project's new chats on the pair you choose, and finds OpenCode's newest models without a relaunch. Apple silicon, macOS 26 or later. Signed and notarized.
...
Swarm Code 1.4.1 folds every finished turn into its summary, keeps the app up through a layout crash, and makes Hydra heads on Z.ai think at the working effort they were promised. Apple silicon, macOS 26 or later. Signed and notarized.
...
Swarm Code 1.4.0 brings Z.ai's reset cards into the usage popover and lets a drag select across a whole reply. Apple silicon, macOS 26 or later. Signed and notarized.
...
Swarm Code 1.3.1 puts the head card in the merge popover where it belongs, shows reply quotes on a sent message as the chips they were in the chat box, and gives the Z.ai mark its colour. Apple silicon, macOS 26 or later. Signed and notarized.
...
Swarm Code 1.3.0 brings Hydra its cookbook: ready-made lead-and-heads pairs in Settings, an intro the first time Hydra is switched on, and popovers that show each merge and report as it lands, alongside Pi and Z.ai joining the providers, banked resets for Codex, slash commands and reply quotes in the composer, and follow-ups that bundle into one message while a turn runs. Apple silicon, macOS 26 or later. Signed and notarized.
...
- A Debug build runs as Swarm Code Dev beside the release app, with its own library and settings.
...
Swarm Code 1.1.0 updates itself from here on: Settings › About checks for new versions and installs them with Update & restart. Apple silicon, macOS 26 or later. Signed and notarized.

### New features
- Swarm Code updates itself: About checks GitLab for new versions, shows what changed, and Update & restart installs it in place.
...
The coding app by Swarm (https://getswarm.app): a native Mac app for your coding agents. Apple silicon, macOS 26 or later. Signed and notarized.
