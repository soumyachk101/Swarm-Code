...
/// Reads scripts from a project's `t3.json`, the same file T3 Code uses.
enum ProjectFile {
    static func scripts(at url: URL) -> [ProjectScript] {
        guard let data = try? Data(contentsOf: url.appendingPathComponent("t3.json")),
