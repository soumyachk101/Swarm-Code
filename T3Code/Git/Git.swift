...
struct Git: Sendable {
    ...
    static func checkpointRef(thread: UUID, turn: Int, phase: String) -> String {
        "refs/t3code/checkpoints/\(thread.uuidString.lowercased())/\(turn)-\(phase)"
    }
    ...
    func captureCheckpoint(_ ref: String) async throws {
        ...
        let index = fileManager.temporaryDirectory.appendingPathComponent("t3code-index-\(UUID().uuidString)")
        ...
        try Self.check(tree)
        let identity = [
            "GIT_AUTHOR_NAME": "T3 Code", "GIT_AUTHOR_EMAIL": "t3code@localhost",
            "GIT_COMMITTER_NAME": "T3 Code", "GIT_COMMITTER_EMAIL": "t3code@localhost",
        ]
        let commit = try await run(["commit-tree", tree.trimmedOutput, "-m", "T3 Code checkpoint"], environment: identity)
    ...
    func deleteCheckpoints(thread: UUID) async {
        let prefix = "refs/t3code/checkpoints/\(thread.uuidString.lowercased())/"
