...
final class CodexSession: ProviderSession {
    ...
    private static func connect(
        ...
        _ = try await connection.request("initialize", [
            "clientInfo": ["name": "t3code", "title": "T3 Code", "version": .string(AppInfo.version)],
    ...
    private func handleRequest(_ id: RPCID, _ method: String, _ params: JSONValue) {
        ...
        case "mcpServer/elicitation/request":
            ...
            onEvent?(.notice(Notice(level: .warning, message: "\(server) asked for input, which T3 Code cannot show yet.")))
        default:
            connection?.respond(to: id, errorCode: -32601, message: "T3 Code does not support \(method).")
        }
    }
