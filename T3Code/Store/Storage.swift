...
enum Storage {
    static let root: URL = {
        ...
        let url = base.appendingPathComponent("T3 Code", isDirectory: true)
    ...
    static var worktreesDirectory: URL {
        let url = URL(fileURLWithPath: LoginEnvironment.homeDirectory)
            .appendingPathComponent(".t3code/worktrees", isDirectory: true)
        try? FileManager.default.createDirectory(at: url, withIntermediateDirectories: true)
        return url
    }
...
actor DiskWriter {
    ...
    func write(_ data: Data, to url: URL) {
        do {
            ...
            NSLog("T3 Code could not save %@: %@", url.lastPathComponent, error.localizedDescription)
        }
    }
}
