...
/**
 * The environment's theme, set with `swarm-code theme set <id>`. Each client applies
...
// Deliberately absent from ServerSettingsPatch: `swarm-code theme set` checks that an
...
export const CodexSettings = makeProviderSettingsSchema(
  {
    ...
    shadowHomePath: TrimmedString.pipe(
      ...
      Schema.annotateKey({
        ...
        providerSettingsForm: {
          placeholder: "~/.codex-swarmcode/personal",
          clearWhenEmpty: "omit",
        },
      }),
    ),
...
export const OpenCodeSettings = makeProviderSettingsSchema(
  {
    ...
    serverUrl: TrimmedString.pipe(
      ...
      Schema.annotateKey({
        title: "Server URL",
        description: "Leave blank to let Swarm Code spawn the server when needed.",
        providerSettingsForm: {
...
export const ServerSettings = Schema.Struct({
   ...
   * withholds the MCP credential, so the `swarm-code` server (and with it every
  ...
  /**
   * Whether this server may install and run Swarm Code's device helper processes.
   ...
   * of the same value from one they already applied: `swarm-code theme set` must act
