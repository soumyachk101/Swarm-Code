...
>()("@swarmcode/contracts/environmentHttp/EnvironmentAuthenticatedPrincipal") {}
...
class EnvironmentMetadataHttpApi extends HttpApiGroup.make("metadata").add(
  HttpApiEndpoint.get("descriptor", "/.well-known/swarmcode/environment", {
...
class EnvironmentConnectHttpApi extends HttpApiGroup.make("connect")
  ...
  .add(
    HttpApiEndpoint.post("health", "/api/swarm-connect/health", {
  ...
  .add(
    HttpApiEndpoint.post("t3MintCredential", "/api/swarm-connect/mint-credential", {
      payload: RelayCloudMintCredentialRequest,
      success: RelayEnvironmentMintResponse,
      error: EnvironmentHttpCloudErrors,
    }),
