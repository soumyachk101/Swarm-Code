...
}).annotate({ description: "Links an authenticated cloud user to a Swarm Code environment." });
...
>()("@swarmcode/contracts/relay/RelayClientPrincipal") {}
...
>()("@swarmcode/contracts/relay/RelayEnvironmentPrincipal") {}

const RelayClientBearerAuthorization = HttpApiSecurity.http({ scheme: "bearer" }).pipe(
  HttpApiSecurity.annotate(
    OpenApi.Description,
    "Clerk session or OAuth bearer token for the signed-in Swarm Code Connect user.",
  ),
);
...
export const RelayApi = HttpApi.make("RelayApi")
  ...
  .annotate(OpenApi.Title, "Swarm Code Relay API")
  .annotate(OpenApi.Version, "1.0.0")
  .annotate(
    OpenApi.Description,
    "Control-plane API for linking Swarm Code environments, connecting authorized clients, and publishing agent activity.",
  );
