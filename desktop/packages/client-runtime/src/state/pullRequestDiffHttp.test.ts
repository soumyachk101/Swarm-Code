import { EnvironmentId, ProjectId } from "@swarmcode/contracts";
  ...
  it.effect("gives rejected diff sessions a recovery action", () =>
    Effect.gen(function* () {
      ...
      expect(error.message).toBe(
        "This environment session is no longer valid (invalid_credential). Refresh the page or quit and reopen Swarm Code.",
      );
    }),
  );
});
