...
import { ThreadId } from "@swarmcode/contracts";
...
describe("resolveWorkEntryToolPresentation", () => {
  ...
  it.each([
    ...
    ["t3_thread_read", "Reading a a Swarm Code thread", "Read a a Swarm Code thread"],
    ["t3_thread_send", "Sending to a a Swarm Code thread", "Sent to a a Swarm Code thread"],
    [
      "t3_worktree_handoff",
...
describe("pull request tool presentation", () => {
  ...
  it("summarizes native PR work separately from ordinary tools and integration metadata", () => {
    const link: WorkLogPresentationEntry = {
      ...
      itemType: "mcp_tool_call",
      toolLifecycleStatus: "completed",
      toolSource: { key: "t3-code", name: "Swarm Code", kind: "integration" },
    };
