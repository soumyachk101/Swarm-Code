...
export class SshPasswordPrompt extends Context.Service<SshPasswordPrompt, SshPasswordPromptShape>()(
  "@swarmcode/ssh/auth/SshPasswordPrompt",
) {
...
# Invoked by ssh via SSH_ASKPASS when Swarm Code re-runs ssh with a cached password
# from the renderer's in-app prompt. We never expose a native dialog here - if
# SWARMCODE_SSH_AUTH_SECRET is missing, that's a caller bug and we fail loudly.
if [ "\${SWARMCODE_SSH_AUTH_SECRET+x}" = "x" ]; then
  printf "%s\\n" "$SWARMCODE_SSH_AUTH_SECRET"
  exit 0
fi
printf 'Swarm Code ssh-askpass invoked without SWARMCODE_SSH_AUTH_SECRET.\\n' >&2
exit 1
`;
...
const ASKPASS_WINDOWS_SCRIPT = `# Invoked by ssh via SSH_ASKPASS (through ssh-askpass.cmd) when Swarm Code re-runs\r
# ssh with a cached password from the renderer's in-app prompt. We never expose\r
# a native dialog here - if SWARMCODE_SSH_AUTH_SECRET is missing, that's a caller bug\r
# and we fail loudly.\r
if ($null -ne $env:SWARMCODE_SSH_AUTH_SECRET) {\r
  [Console]::Out.WriteLine($env:SWARMCODE_SSH_AUTH_SECRET)\r
  exit 0\r
}\r
[Console]::Error.WriteLine("Swarm Code ssh-askpass invoked without SWARMCODE_SSH_AUTH_SECRET.")\r
exit 1\r
`;
...
> {
  ...
  return {
    ...
    ...(input.authSecret === undefined ? {} : { SWARMCODE_SSH_AUTH_SECRET: input.authSecret ?? "" }),
