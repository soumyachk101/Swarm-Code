...
describe("Clerk relay auth", () => {
  it("derives a custom Frontend API hostname from a Clerk publishable key", () => {
    expect(clerkFrontendApiHostnameFromPublishableKey(clerkPublishableKey("clerk.swarmcode.vercel.app"))).toBe(
      "clerk.swarmcode.vercel.app",
    );
    expect(clerkFrontendApiUrlFromPublishableKey(clerkPublishableKey("clerk.swarmcode.vercel.app"))).toBe(
      "https://clerk.swarmcode.vercel.app",
    );
  });
  ...
  it("reports semantic frontend API failures without inventing a cause", () => {
    ...
    const pathFrontendApi = "clerk.swarmcode.vercel.app/path";
