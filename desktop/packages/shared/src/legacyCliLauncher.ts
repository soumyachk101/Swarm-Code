/**
 * `dist/bin.mjs` of the `swarmcode` npm package: the entry point boot-service
 ...
 * executable in the sibling `@swarmcode/swarmcode-<platform>-<arch>` package.
 *
...
const executableName = process.platform === "win32" ? "swarmcode.exe" : "swarmcode";
const executable = join(dirname(require.resolve("@swarmcode/swarmcode-" + process.platform + "-" + process.arch + "/package.json")), executableName);
...
const fail = (error) => {
  if (!error) return;
  process.stderr.write("swarmcode: " + error.message + "\n");
  child.kill("SIGTERM");
  process.exitCode = 1;
};
if (ipc) {
