...
import { resolveGitWorktreePath, resolveWorktreeSwarmCodeHome } from "./devHome.ts";
...
describe("resolveWorktreeSwarmCodeHome", () => {
  it.effect("answers with .swarmcode before the dev runner creates it", () =>
    Effect.gen(function* () {
      const { root, nested } = yield* makeRepo("worktree");
      const home = yield* resolveWorktreeSwarmCodeHome(nested);
      assert.equal(home, NodePath.join(NodePath.resolve(root), ".swarmcode"));
