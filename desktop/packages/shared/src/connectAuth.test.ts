...
describe("connectAuth", () => {
  it("round-trips state, challenge, and loopback port through the authorize URL fragment", () => {
    const url = buildConnectAuthorizeRequestUrl({
      hostedAppUrl: "https://swarmcode.vercel.app",
    ...
    expect(parsed.origin).toBe("https://swarmcode.vercel.app");
  ...
  it("rejects authorize requests missing state, challenge, or port", () => {
    expect(readConnectAuthorizeRequest(new URL("https://swarmcode.vercel.app/connect"))).toBeNull();
    expect(
      readConnectAuthorizeRequest(new URL("https://swarmcode.vercel.app/connect#state=abc&port=34338")),
    ).toBeNull();
    expect(
      readConnectAuthorizeRequest(new URL("https://swarmcode.vercel.app/connect#challenge=abc&port=34338")),
    ).toBeNull();
    expect(
      readConnectAuthorizeRequest(new URL("https://swarmcode.vercel.app/connect#state=abc&challenge=abc")),
    ).toBeNull();
  });
  ...
  it("rejects authorize requests whose loopback port is corrupted", () => {
    for (const port of ["", "abc", "-1", "0", "65536", "34338x", "34 38"]) {
      const url = new URL(
        `https://swarmcode.vercel.app/connect#state=state-1&challenge=challenge-1&port=${encodeURIComponent(port)}`,
      );
  ...
  it("builds a PKCE authorize URL against the Clerk endpoint", () => {
    const url = new URL(
      buildConnectClerkAuthorizeUrl({
        authorizationEndpoint: "https://clerk.swarmcode.vercel.app/oauth/authorize",
    ...
    expect(url.origin).toBe("https://clerk.swarmcode.vercel.app");
