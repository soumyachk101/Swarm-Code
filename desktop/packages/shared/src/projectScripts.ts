import type { ProjectId, ProjectScript, ServerSettings } from "@swarmcode/contracts";
...
export function projectScriptRuntimeEnv(
  ...
  const env: Record<string, string> = {
    SWARMCODE_PROJECT_ROOT: input.project.cwd,
  };
  if (input.worktreePath) {
    env.SWARMCODE_WORKTREE_PATH = input.worktreePath;
  }
  if (input.extraEnv) {
