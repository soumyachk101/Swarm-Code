...
import * as NetService from "@swarmcode/shared/Net";
import { resolveGitWorktreePath, resolveWorktreeSwarmCodeHome } from "@swarmcode/shared/devHome";
import { HostProcessEnvironment, HostProcessWorkingDirectory } from "@swarmcode/shared/hostProcess";
...
export const DEFAULT_SWARMCODE_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(NodeOS.homedir(), ".swarmcode"),
);

const MODE_ARGS = {
  dev: [
    "run",
    "--filter=@swarmcode/contracts",
    "--filter=@swarmcode/web",
    "--filter=swarmcode",
    "--parallel",
    "dev",
  ],
  "dev:server": ["run", "--filter=swarmcode", "dev"],
  "dev:web": ["run", "--filter=@swarmcode/web", "dev"],
  "dev:desktop": ["run", "--filter=@swarmcode/desktop", "--filter=@swarmcode/web", "dev"],
} as const satisfies Record<string, ReadonlyArray<string>>;

type DevMode = keyof typeof MODE_ARGS;
/**
 * `role` matters because only the backend honours `--host`/`SWARMCODE_HOST`; the
...
export class DevRunnerInvalidPortOffsetError extends Schema.TaggedError<DevRunnerInvalidPortOffsetError>()(
  ...
  {
    configKey: Schema.Literal("SWARMCODE_PORT_OFFSET"),
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("SWARMCODE_PORT_OFFSET"),
  devInstance: optionalStringConfig("SWARMCODE_DEV_INSTANCE"),
});
...
> {
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      return Effect.fail(
        new DevRunnerInvalidPortOffsetError({
          configKey: "SWARMCODE_PORT_OFFSET",
    ...
    return Effect.succeed({
      ...
      source: `SWARMCODE_PORT_OFFSET=${config.portOffset}`,
    });
  }
  ...
  if (seed) {
    if (/^\d+$/.test(seed)) {
      return Effect.succeed({
        offset: Number(seed),
        source: `numeric SWARMCODE_DEV_INSTANCE=${seed}`,
      });
    }

    const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
    return Effect.succeed({ offset, source: `hashed SWARMCODE_DEV_INSTANCE=${seed}` });
  }
...
function resolveBaseDir(baseDir: string | undefined): Effect.Effect<string, never, Path.Path> {
  return Effect.gen(function* () {
    ...
    return yield* DEFAULT_SWARMCODE_HOME;
  });
}
...
interface CreateDevRunnerEnvInput {
  ...
  readonly swarmCodeHome: string | undefined;
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    // Precedence (--home-dir > worktree .t3 > ambient SWARMCODE_HOME) is resolved
    // by the caller; an unset t3Home here genuinely means "use the default".
    const configuredBaseDir = swarmCodeHome?.trim() || undefined;
    ...
    if (configuredBaseDir !== undefined) {
      output.SWARMCODE_HOME = resolvedBaseDir;
    } else {
      delete output.SWARMCODE_HOME;
    }

    // A dev-runner server is never launcher-managed. When the shell that runs
    // this script was itself spawned by the machine's managed swarm-code service (an
    // agent working inside Swarm Code), these leak through and the child server
    // fails startup with "The service launcher started a different swarm-code version"
    // (serviceLauncherClient.ts resolveStartup).
    delete output.SWARMCODE_SERVICE_LAUNCHER_CONTEXT;
    delete output.SWARMCODE_BOOT_SERVICE_UNIT;

    if (!isDesktopMode) {
      output.SWARMCODE_PORT = String(serverPort);
      ...
      if (mode === "dev" || mode === "dev:web") {
        ...
        output.SWARMCODE_SINGLE_ORIGIN_DEV = "1";
      } else {
        output.VITE_HTTP_URL = `http://localhost:${serverPort}`;
        output.VITE_WS_URL = `ws://localhost:${serverPort}`;
        delete output.SWARMCODE_SINGLE_ORIGIN_DEV;
      }
    } else {
      output.SWARMCODE_PORT = String(serverPort);
      ...
      delete output.SWARMCODE_SINGLE_ORIGIN_DEV;
      delete output.SWARMCODE_MODE;
      delete output.SWARMCODE_NO_BROWSER;
      delete output.SWARMCODE_HOST;
      delete output.SWARMCODE_DEV_AUTH_TOKEN;
    }

    if (!isDesktopMode && host !== undefined) {
      output.SWARMCODE_HOST = host;
    }

    if (!isDesktopMode) {
      output.SWARMCODE_NO_BROWSER = browser === true ? "0" : "1";
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.SWARMCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.SWARMCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.SWARMCODE_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.SWARMCODE_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.SWARMCODE_MODE = "web";
      delete output.SWARMCODE_DESKTOP_WS_URL;
    }
 ...
 * target reach it there. When `--host`/`SWARMCODE_HOST` moves the backend onto
...
export function runDevRunnerWithInput(input: DevRunnerCliInput) {
  return Effect.gen(function* () {
      ...
      Effect.mapError(
        (cause) =>
          new DevRunnerConfigurationError({
            configKeys: ["SWARMCODE_PORT_OFFSET", "SWARMCODE_DEV_INSTANCE"],
            cause,
          }),
      ),
    );
    ...
    // (gitignored) `.t3` — see @swarmcode/shared/devHome for why this must
    // outrank an ambient SWARMCODE_HOME. `--home-dir` still wins.
    const worktreeHome = yield* resolveWorktreeSwarmCodeHome(yield* HostProcessWorkingDirectory);
    ...
    const resolvedSwarmCodeHome =
      (input.swarmCodeHome?.trim() || undefined) ??
      worktreeHome ??
      (hostEnvironment.SWARMCODE_HOME?.trim() || undefined);
    const env = yield* createDevRunnerEnv({
      mode: input.mode,
      baseEnv: hostEnvironment,
      serverOffset,
      webOffset,
      swarmCodeHome: resolvedSwarmCodeHome,
    ...
    const selectionSuffix =
      serverOffset !== offset || webOffset !== offset
        ...
        : "";
    const baseDir = env.SWARMCODE_HOME ?? (yield* DEFAULT_SWARMCODE_HOME);

    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.SWARMCODE_PORT)} webPort=${String(env.PORT)} baseDir=${baseDir}`,
    );
    ...
    if (input.share) {
      if (input.mode === "dev:server") {
        ...
        if (shared) {
          ...
          env.SWARMCODE_DEV_ALLOWED_ORIGINS = [
            env.SWARMCODE_DEV_ALLOWED_ORIGINS,
          ...
          // a few chunk requests. Only defaulted, so SWARMCODE_BUNDLED_DEV=0
          // still opts a --share run back out.
          if (env.SWARMCODE_BUNDLED_DEV === undefined) {
            env.SWARMCODE_BUNDLED_DEV = "1";
          }
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  swarmCodeHome: Flag.String("home-dir").pipe(
    Flag.withDescription(
      "Explicit Swarm Code data directory; runtime state is stored under userdata (equivalent to SWARMCODE_HOME). Inside a git worktree this defaults to that worktree's own .swarm-code so dev state stays off the shared home.",
    ),
    Flag.optional,
  ...
  autoBootstrapProjectFromCwd: Flag.Boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to SWARMCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("SWARMCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.Boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to SWARMCODE_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("SWARMCODE_LOG_WS_EVENTS")),
  ),
  host: Flag.String("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to SWARMCODE_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("SWARMCODE_HOST")),
  ),
  port: Flag.Int("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to SWARMCODE_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("SWARMCODE_PORT")),
  ),
