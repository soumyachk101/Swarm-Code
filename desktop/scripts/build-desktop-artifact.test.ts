...
import { symlinksSupported } from "@swarmcode/shared/testing/symlinks";
...
const makeWindowsPayloadFixture = Effect.fn("test.makeWindowsPayloadFixture")(function* (input: {
  ...
  const tempDir = yield* fs.makeTempDirectoryScoped({
    prefix: "swarm-code-windows-payload-test-",
  });
  ...
  yield* fs.writeFileString(
    path.join(resourcesDir, "resource-monitor/swarm-code-resource-monitor.exe"),
    "monitor",
  );
  const appExecutableName = "swarm-code.exe";
...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it("switches desktop packaging product names to nightly for nightly builds", () => {
    assert.equal(resolveDesktopProductName("0.0.17"), "Swarm Code (Alpha)");
    assert.equal(resolveDesktopProductName("0.0.17-nightly.20260413.42"), "Swarm Code (Nightly)");
  });
        ...
        Effect.provide(
          ConfigProvider.layer(
            ConfigProvider.fromEnv({
              env: {
                SWARMCODE_DESKTOP_UPDATE_REPOSITORY: "pingdotgg/swarm-code",
              },
            }),
          ),
        ),
      ...
      const nightlyConfig = yield* resolveGitHubPublishConfig("nightly").pipe(
        Effect.provide(
          ConfigProvider.layer(
            ConfigProvider.fromEnv({
              env: {
                GITHUB_REPOSITORY: "pingdotgg/swarm-code",
              },
            }),
          ),
        ),
  ...
  it.effect("omits update feeds for pull request preview builds", () =>
    Effect.gen(function* () {
      ...
      Effect.provide(
        ConfigProvider.layer(
          ConfigProvider.fromEnv({ env: { GITHUB_REPOSITORY: "pingdotgg/swarm-code" } }),
        ),
      ),
    ),
  );
      ...
      resolveDesktopRuntimeDependencies(
        {
          ...
          "@swarmcode/contracts": "workspace:*",
          "@swarmcode/shared": "workspace:*",
          "dbus-next": "0.10.2",
          effect: "catalog:",
          electron: "41.5.0",
  ...
  it.effect("applies platform-specific packaging to the build config", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(mac.dmg, {
        title: "Swarm Code (Alpha) 1.2.3 Installer",
      ...
      // entry advertises MimeType=x-scheme-handler/swarm-code; for OAuth deep links.
      assert.deepStrictEqual((linux.linux as Record<string, unknown>).protocols, [
        { name: "Swarm Code", schemes: ["swarm-code", "swarm-code-dev"] },
      ]);
  ...
  it.effect("keeps target native files while excluding the other Windows architecture", () =>
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const path = yield* Path.Path;
        const tempDir = yield* fs.makeTempDirectoryScoped({
          prefix: "swarm-code-windows-architecture-test-",
        });
    ...
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const repoRoot = yield* fs.makeTempDirectoryScoped({
          prefix: "swarm-code-resource-monitor-cache-test-",
        });
        const binaryPath = path.join(
          repoRoot,
          "native/resource-monitor/target/x86_64-unknown-linux-gnu/release/swarm-code-resource-monitor",
        );
        ...
        yield* stageResourceMonitor({
          ...
          Effect.provide(
            ConfigProvider.layer(
              ConfigProvider.fromEnv({
                env: { SWARMCODE_DESKTOP_REUSE_RESOURCE_MONITOR: "true" },
              }),
            ),
          ),
        );
        ...
        assert.equal(
          yield* fs.readFileString(
            path.join(stageResourcesDir, "resource-monitor/swarm-code-resource-monitor"),
          ),
          "cached monitor",
        );
      }),
  ...
  it.effect("reports missing Windows toolchain capabilities before building", () =>
    Effect.scoped(
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
        const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "swarm-code-windows-preflight-" });
              ...
              ConfigProvider.layer(
                ConfigProvider.fromEnv({
                  env: {
                    ...
                    SWARMCODE_DESKTOP_REUSE_RESOURCE_MONITOR: "true",
                  },
                }),
              ),
            ),
  ...
  it.effect("rejects a PATH-discovered Python executable that is not Python 3", () =>
    Effect.scoped(
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
        const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "swarm-code-python2-preflight-" });
  ...
  it.effect("builds and stages native capture helpers for each Linux architecture", () =>
    Effect.scoped(
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
        const repoRoot = yield* fs.makeTempDirectoryScoped({ prefix: "swarm-code-kde-stage-test-" });
        ...
        for (const backend of ["kde", "hyprland"] as const) {
          for (const [arch, target] of [
            ...
            const binary = path.join(
              ...
              target,
              `release/swarm-code-${backend}-snap-shot`,
            );
            ...
            const installed = path.join(
              stageResourcesDir,
              `${backend}-capture/swarm-code-${backend}-snap-shot`,
            );
  ...
  // The fixture's swarm-code.exe is a text placeholder, not an executable. These
  ...
  it.effect("builds the Linux browser secret helper for a concrete architecture", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.deepStrictEqual(helper.args.slice(-4), [
        ...
        "x64",
        "--output",
        path.join("/stage/resources", "browser-secret", "swarm-code-browser-secret"),
      ]);
    }).pipe(
      Effect.provide(
        Layer.mergeAll(
    ...
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const resourceMonitorPath = path.join(
          fixture.packagedAppDir,
          "resources/resource-monitor/swarm-code-resource-monitor.exe",
        );
        ...
        assert.deepStrictEqual(resourceMonitorError.missingFiles, [
          "resource-monitor/swarm-code-resource-monitor.exe",
        ]);
      }),
    ),
  );
  ...
  it.effect("rejects a sidecar whose extracted server bundle cannot resolve", () =>
    Effect.scoped(
      Effect.gen(function* () {
        const fixture = yield* makeWindowsPayloadFixture({
          copyUnpackedNatives: true,
          serverEntrySource: 'import "swarm-code-deliberately-missing-package";\n',
        });
        const error = yield* validateWindowsPackagedPayload({
          stageDistDir: fixture.stageDistDir,
          appExecutableName: fixture.appExecutableName,
        ...
        assert.include(error.output, "swarm-code-deliberately-missing-package");
      }),
    ).pipe(Effect.provideService(HostProcessPlatform, "linux")),
  );
    ...
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const stageResourcesDir = yield* fs.makeTempDirectoryScoped({
          prefix: "swarm-code-dmg-background-",
        });
  ...
  it.effect("fails clearly when the selected DMG background source is missing", () =>
    Effect.scoped(
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const stageResourcesDir = yield* fs.makeTempDirectoryScoped({
          prefix: "swarm-code-dmg-background-missing-",
        });
  ...
  it("derives macOS passkey signing configuration from the Clerk publishable key", () => {
    const configuration = resolveMacPasskeySigningConfiguration({
      SWARMCODE_APPLE_TEAM_ID: "abc1234567",
      SWARMCODE_MACOS_PROVISIONING_PROFILE: "/tmp/swarm-code.provisionprofile",
      SWARMCODE_CLERK_PUBLISHABLE_KEY: `pk_test_${btoa("example.clerk.accounts.dev$")}`,
    });

    assert.deepStrictEqual(configuration, {
      appId: "com.swarmcode.swarm-code",
      teamId: "ABC1234567",
      rpDomains: ["example.clerk.accounts.dev"],
      provisioningProfilePath: "/tmp/swarm-code.provisionprofile",
    });
  });
  ...
  it("normalizes explicit macOS passkey RP domains and renders required entitlements", () => {
    const configuration = resolveMacPasskeySigningConfiguration({
      SWARMCODE_APPLE_TEAM_ID: "ABC1234567",
      SWARMCODE_MACOS_PROVISIONING_PROFILE: "/tmp/swarm-code.provisionprofile",
      SWARMCODE_CLERK_PASSKEY_RP_DOMAINS:
    ...
    assert.include(entitlements, "<string>ABC1234567.com.swarmcode.swarm-code</string>");
  ...
  it("rejects incomplete macOS passkey signing configuration", () => {
    ...
    const missingProfileError = captureError({
      SWARMCODE_APPLE_TEAM_ID: "ABC1234567",
      SWARMCODE_CLERK_PASSKEY_RP_DOMAINS: "example.clerk.accounts.dev",
    });
    assert.instanceOf(missingProfileError, MissingMacPasskeyProvisioningProfileError);
    assert.equal(
      missingProfileError.message,
      "SWARMCODE_MACOS_PROVISIONING_PROFILE must point to an Associated Domains provisioning profile.",
    );
    ...
    const invalidDomainError = captureError({
      SWARMCODE_APPLE_TEAM_ID: "ABC1234567",
      SWARMCODE_MACOS_PROVISIONING_PROFILE: "/tmp/swarm-code.provisionprofile",
      SWARMCODE_CLERK_PASSKEY_RP_DOMAINS: unsafeDomain,
    });
    ...
    assert.throws(
      () =>
        resolveMacPasskeySigningConfiguration({
          SWARMCODE_APPLE_TEAM_ID: "ABC1234567",
          SWARMCODE_MACOS_PROVISIONING_PROFILE: "/tmp/swarm-code.provisionprofile",
          SWARMCODE_CLERK_PASSKEY_RP_DOMAINS: "example.clerk.accounts.dev:8443",
        }),
    ...
    const invalidPublishableKeyError = captureError({
      SWARMCODE_APPLE_TEAM_ID: "ABC1234567",
      SWARMCODE_MACOS_PROVISIONING_PROFILE: "/tmp/swarm-code.provisionprofile",
      SWARMCODE_CLERK_PUBLISHABLE_KEY: "pk_test_%",
    });
    ...
    assert.equal(invalidPublishableKeyError.message, "SWARMCODE_CLERK_PUBLISHABLE_KEY is invalid.");
  ...
  it.effect("adds passkey entitlements and both renderer protocols to signed macOS builds", () =>
    Effect.gen(function* () {
      ...
      assert.equal(config.appId, "com.swarmcode.swarm-code");
      assert.equal(mac.entitlements, "/tmp/entitlements.mac.plist");
      assert.equal(mac.provisioningProfile, "/tmp/swarm-code.provisionprofile");
  ...
  it("stages the resource monitor as an external executable resource", () => {
    ...
    assert.equal(resourceMonitorExecutableName("mac"), "swarm-code-resource-monitor");
    assert.equal(resourceMonitorExecutableName("win"), "swarm-code-resource-monitor.exe");
  });
  ...
  it("ships the Linux CLI release archive as the WSL runtime", () => {
    ...
    const runtimeArchivePath = "/tmp/swarm-code-1.2.3-linux-x64.tar.gz";
    ...
    assert.equal(wslRuntimeArchiveStem("1.2.3", "x64"), "swarm-code-1.2.3-linux-x64");
  });

  it("parses Windows bsdtar member listings with CRLF line endings", () => {
    assert.deepStrictEqual(
      parseWslRuntimeArchiveMembers("./swarm-code-1.2.3-linux-x64/t3\r\nswarm-code-1.2.3-linux-x64/client/\r\n"),
      ["swarm-code-1.2.3-linux-x64/t3", "swarm-code-1.2.3-linux-x64/client"],
    );
  });
  ...
  it.effect("stages the Linux CLI archive verbatim with its SHA-256 sidecar", () =>
    Effect.scoped(
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
        const root = yield* fs.makeTempDirectoryScoped({ prefix: "swarm-code-wsl-runtime-stage-" });
        const sourceArchivePath = yield* makeLinuxCliArchiveFixture({
          root,
          stem: "swarm-code-1.2.3-linux-x64",
        });
  ...
  it.effect("fails when the Linux CLI archive handed to the build does not exist", () =>
    Effect.scoped(
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
        const root = yield* fs.makeTempDirectoryScoped({ prefix: "swarm-code-wsl-runtime-missing-" });
        const error = yield* stageWslRuntimeArchive({
          sourceArchivePath: path.join(root, "swarm-code-1.2.3-linux-x64.tar.gz"),
      ...
      const resolved = yield* resolveBuildOptions({
        ...
        Effect.provide(
          ConfigProvider.layer(
            ConfigProvider.fromEnv({
              env: {
                SWARMCODE_DESKTOP_SKIP_BUILD: "true",
                SWARMCODE_DESKTOP_KEEP_STAGE: "true",
                SWARMCODE_DESKTOP_SIGNED: "true",
                SWARMCODE_DESKTOP_VERBOSE: "true",
                SWARMCODE_DESKTOP_MOCK_UPDATES: "true",
              },
            }),
          ),
        ),
...
it.effect.skipIf(!symlinksSupported)("rebases packaged links into the isolated tree", () =>
  Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const root = yield* fs.makeTempDirectoryScoped({ prefix: "swarm-code-copy-symlinks-" });
