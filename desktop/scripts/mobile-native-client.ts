...
import { isCommandAvailable, resolveSpawnCommand } from "@swarmcode/shared/shell";
...
const bundleId = "com.swarmcode.swarm-code.dev";
...
const fingerprint = Effect.fn("nativeClient.fingerprint")(function* (platform: NativePlatform) {
  const output = yield* command(yield* HostProcessExecutablePath, [
    "--eval",
    `require('expo/fingerprint').createFingerprintAsync(process.cwd(), { platforms: [process.argv[1]], silent: true }).then(fp => console.log('SWARM_CODE_NATIVE_FINGERPRINT=' + fp.hash)).catch(e => { console.error(e); process.exitCode = 1; });`,
    platform,
  ]);
  const hash = output
    .split("\n")
    .find((line) => line.startsWith("SWARM_CODE_NATIVE_FINGERPRINT="))
    ?.split("=")[1];
...
const main = Command.make(
  ...
  Effect.fn("nativeClient.main")(function* ({ mode, platform, device }) {
    ...
    const recordPath = path.join(
      home,
      ".cache/swarm-code/native-clients",
      platform,
    ...
    const operations = {
      ...
      build: Effect.gen(function* () {
        ...
        if (platform === "ios") {
          const output = yield* fs.makeTempDirectoryScoped({ prefix: "swarm-code-native-client-" });
          ...
          yield* command(
            ...
            [
              "xcodebuild",
              "-workspace",
              path.join(mobile, "ios/swarm-codeDev.xcworkspace"),
              "-scheme",
              "swarm-codeDev",
              "-configuration",
              "Debug",
          ...
          yield* command(
            ...
            [
              ...
              "install",
              device,
              path.join(output, "Build/Products/Debug-iphonesimulator/swarm-codeDev.app"),
            ],
            true,
          );
        } else {
