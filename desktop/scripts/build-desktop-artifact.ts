...
import { fromYaml } from "@swarmcode/shared/schemaYaml";
import { HostProcessArchitecture, HostProcessPlatform } from "@swarmcode/shared/hostProcess";
import { clerkFrontendApiHostnameFromPublishableKey } from "@swarmcode/shared/relayAuth";
import { resolveSpawnCommand } from "@swarmcode/shared/shell";
...
const DESKTOP_APP_ID = "com.swarmcode.swarm-code";
...
export function resourceMonitorExecutableName(platform: typeof BuildPlatform.Type): string {
  return platform === "win" ? "swarm-code-resource-monitor.exe" : "swarm-code-resource-monitor";
}
...
interface StagePackageJson {
  ...
  readonly swarmCodeCommitHash: string;
...
export const DESKTOP_FILE_EXCLUSIONS = [
  // Swarm Code always passes the user's installed Claude executable to the SDK,
...
// The WSL runtime is the Linux CLI release archive (swarm-code-<version>-linux-<arch>
...
export class InvalidAppleTeamIdError extends Schema.TaggedError<InvalidAppleTeamIdError>()(
  ...
  override get message(): string {
    return `SWARMCODE_APPLE_TEAM_ID '${this.teamId}' must be a 10-character Apple Developer Team ID.`;
  }
}
...
export class MissingMacPasskeyProvisioningProfileError extends Schema.TaggedError<MissingMacPasskeyProvisioningProfileError>()(
  ...
  override get message(): string {
    return "SWARMCODE_MACOS_PROVISIONING_PROFILE must point to an Associated Domains provisioning profile.";
  }
}
...
export class MissingMacPasskeyDomainConfigurationError extends Schema.TaggedError<MissingMacPasskeyDomainConfigurationError>()(
  ...
  override get message(): string {
    return "SWARMCODE_CLERK_PUBLISHABLE_KEY or SWARMCODE_CLERK_PASSKEY_RP_DOMAINS is required for signed macOS passkey builds.";
  }
}
...
export class InvalidMacPasskeyPublishableKeyError extends Schema.TaggedError<InvalidMacPasskeyPublishableKeyError>()(
  ...
  override get message(): string {
    return "SWARMCODE_CLERK_PUBLISHABLE_KEY is invalid.";
  }
}
...
export function resolveMacPasskeySigningConfiguration(
  ...
  const teamId = env.SWARMCODE_APPLE_TEAM_ID?.trim().toUpperCase() ?? "";
  if (!APPLE_TEAM_ID_PATTERN.test(teamId)) {
    throw new InvalidAppleTeamIdError({ teamId });
  }

  const provisioningProfilePath = env.SWARMCODE_MACOS_PROVISIONING_PROFILE?.trim() ?? "";
  if (provisioningProfilePath.length === 0) {
    throw new MissingMacPasskeyProvisioningProfileError();
  }

  const configuredRpDomains = env.SWARMCODE_CLERK_PASSKEY_RP_DOMAINS?.trim();
  let rpDomains: readonly string[];
  if (configuredRpDomains) {
    rpDomains = configuredRpDomains.split(",").map(normalizePasskeyRpDomain);
  } else {
    const publishableKey = env.SWARMCODE_CLERK_PUBLISHABLE_KEY?.trim();
    if (!publishableKey) {
...
const BuildEnvConfig = Config.all({
  platform: Config.schema(BuildPlatform, "SWARMCODE_DESKTOP_PLATFORM").pipe(Config.option),
  target: Config.String("SWARMCODE_DESKTOP_TARGET").pipe(Config.option),
  arch: Config.schema(BuildArch, "SWARMCODE_DESKTOP_ARCH").pipe(Config.option),
  version: Config.String("SWARMCODE_DESKTOP_VERSION").pipe(Config.option),
  outputDir: Config.String("SWARMCODE_DESKTOP_OUTPUT_DIR").pipe(Config.option),
  skipBuild: Config.Boolean("SWARMCODE_DESKTOP_SKIP_BUILD").pipe(Config.withDefault(false)),
  keepStage: Config.Boolean("SWARMCODE_DESKTOP_KEEP_STAGE").pipe(Config.withDefault(false)),
  signed: Config.Boolean("SWARMCODE_DESKTOP_SIGNED").pipe(Config.withDefault(false)),
  verbose: Config.Boolean("SWARMCODE_DESKTOP_VERBOSE").pipe(Config.withDefault(false)),
  mockUpdates: Config.Boolean("SWARMCODE_DESKTOP_MOCK_UPDATES").pipe(Config.withDefault(false)),
  mockUpdateServerPort: Config.String("SWARMCODE_DESKTOP_MOCK_UPDATE_SERVER_PORT").pipe(Config.option),
  // Path to the Linux CLI release archive (swarm-code-<version>-linux-x64.tar.gz) built
  // by the build_linux_cli CI job. The Windows build embeds it verbatim as the
  // WSL runtime.
  wslRuntime: Config.String("SWARMCODE_DESKTOP_WSL_RUNTIME").pipe(Config.option),
});
...
export const preflightLinuxDesktopBuild = Effect.fn("preflightLinuxDesktopBuild")(function* (
  ...
  const reuseResourceMonitor = yield* Config.Boolean("SWARMCODE_DESKTOP_REUSE_RESOURCE_MONITOR").pipe(
    Config.withDefault(false),
  );
  const reuseCaptureHelpers = yield* Config.Boolean(
    "SWARMCODE_DESKTOP_REUSE_LINUX_CAPTURE_HELPERS",
...
export const preflightWindowsDesktopBuild = Effect.fn("preflightWindowsDesktopBuild")(
  function* (input: { readonly arch: typeof BuildArch.Type; readonly bundlesWslRuntime: boolean }) {
    ...
    const reuseResourceMonitor = yield* Config.Boolean(
      "SWARMCODE_DESKTOP_REUSE_RESOURCE_MONITOR",
    ).pipe(Config.withDefault(false));
...
const verifyPackagedBundleIsSelfContained = Effect.fn("verifyPackagedBundleIsSelfContained")(
  function* (input: { readonly asarPath: string; readonly verbose: boolean }) {
    ...
    const probeRoot = yield* fs.makeTempDirectoryScoped({
      prefix: "swarm-code-bundle-selfcheck-",
    });
...
export const stageLinuxCaptureHelper = Effect.fn("stageLinuxCaptureHelper")(function* (input: {
  ...
  const reuseHelpers = yield* Config.Boolean("SWARMCODE_DESKTOP_REUSE_LINUX_CAPTURE_HELPERS").pipe(
    Config.withDefault(false),
  );
  const binaryPath = path.join(
    ...
    rustTarget!,
    `release/swarm-code-${input.backend}-snap-shot`,
  );
  if (!reuseHelpers) {
  ...
  const executable = path.join(destination, `swarm-code-${input.backend}-snap-shot`);
  ...
  yield* runCommand(
    ChildProcess.make(
      ...
      [
        ...
        "--output",
        path.join(input.stageResourcesDir, "browser-secret", "swarm-code-browser-secret"),
      ],
      { cwd: input.repoRoot },
    ),
...
export const resolveGitHubPublishConfig = Effect.fn("resolveGitHubPublishConfig")(function* (
  ...
  const env = yield* Config.all({
    updateRepository: Config.String("SWARMCODE_DESKTOP_UPDATE_REPOSITORY").pipe(Config.option),
...
export function resolveDesktopProductName(version: string): string {
  return resolveDesktopUpdateChannel(version) === "nightly"
    ? "Swarm Code (Nightly)"
    : (desktopPackageJson.productName ?? "Swarm Code");
}
...
export const createBuildConfig = Effect.fn("createBuildConfig")(function* (
  ...
  const buildConfig: Record<string, unknown> = {
    ...
    artifactName: Swarm-Code-${version}-${arch}.${ext}",
    ...
    buildConfig.mac = {
      ...
      extendInfo: {
        NSScreenCaptureUsageDescription:
          "Swarm Code captures the active window when you use the window capture shortcut.",
      },
      protocols: [
        {
          name: "Swarm Code",
          schemes: ["swarm-code", "swarm-code-dev"],
        },
      ],
  ...
  if (platform === "linux") {
    buildConfig.linux = {
      target: [target],
      executableName: "swarm-code",
      icon: "icons",
      category: "Development",
      // electron-builder turns these into MimeType=x-scheme-handler/<scheme>;
      // in the .desktop entry (Exec already gets %U), so browsers can hand
      // swarm-code:// OAuth callbacks to the app.
      protocols: [
        {
          name: "Swarm Code",
          schemes: ["swarm-code", "swarm-code-dev"],
...
export const verifyWindowsPrimaryFffNativeLoad = Effect.fn(
  ...
  const probeRoot = yield* fs.makeTempDirectoryScoped({
    prefix: "swarm-code-windows-primary-native-probe-",
  });
...
export const validateWindowsPackagedPayload = Effect.fn(
  ...
  // directory is named swarm-code-<version>-linux-<arch>.
  readonly appVersion: string;
  ...
  if (!(yield* isFile(resourceMonitorPath))) {
    return yield* new WindowsPackagedPayloadValidationError({
      ...
      packagedAppDir,
      missingFiles: ["resource-monitor/swarm-code-resource-monitor.exe"],
    });
  }
  ...
  if (hasWslArchive && hasWslArchiveHash) {
    ...
    // desktop app's WSL install script relies on that layout to find `swarmcode`.
...
const buildDesktopArtifact = Effect.fn("buildDesktopArtifact")(function* (
  ...
  const stageRoot = yield* mkdir({
    prefix: `swarm-code-desktop-${options.platform}-stage-`,
  });
  ...
  const stagePackageJson: StagePackageJson = {
    ...
    swarmCodeCommitHash: commitHash,
    private: true,
    packageManager: rootPackageJson.packageManager,
    description: "Swarm Code desktop build",
    author: "Swarm Code Tools",
  ...
  yield* runCommand(
    ...
    {
      label: `vp exec --filter @swarmcode/desktop -- electron-builder --projectDir ${stageAppDir} ${platformConfig.cliFlag} --${options.arch} --publish never`,
      verbose: options.verbose,
    },
  );
...
const buildDesktopArtifactCli = Command.make("build-desktop-artifact", {
  platform: Flag.Literals("platform", BuildPlatform.literals).pipe(
    Flag.withDescription("Build platform (env: SWARMCODE_DESKTOP_PLATFORM)."),
    Flag.optional,
  ),
  target: Flag.String("target").pipe(
    Flag.withDescription(
      "Artifact target, for example dmg/AppImage/nsis (env: SWARMCODE_DESKTOP_TARGET).",
    ),
    Flag.optional,
  ),
  arch: Flag.Literals("arch", BuildArch.literals).pipe(
    Flag.withDescription("Build arch, for example arm64/x64/universal (env: SWARMCODE_DESKTOP_ARCH)."),
    Flag.optional,
  ),
  buildVersion: Flag.String("build-version").pipe(
    Flag.withDescription("Artifact version metadata (env: SWARMCODE_DESKTOP_VERSION)."),
    Flag.optional,
  ),
  outputDir: Flag.String("output-dir").pipe(
    Flag.withDescription("Output directory for artifacts (env: SWARMCODE_DESKTOP_OUTPUT_DIR)."),
    Flag.optional,
  ),
  skipBuild: Flag.Boolean("skip-build").pipe(
    Flag.withDescription(
      "Skip `vp run build:desktop` and use existing dist artifacts (env: SWARMCODE_DESKTOP_SKIP_BUILD).",
    ),
    Flag.optional,
  ),
  keepStage: Flag.Boolean("keep-stage").pipe(
    Flag.withDescription("Keep temporary staging files (env: SWARMCODE_DESKTOP_KEEP_STAGE)."),
    Flag.optional,
  ),
  signed: Flag.Boolean("signed").pipe(
    Flag.withDescription(
      "Enable signing/notarization discovery; Windows uses Azure Trusted Signing (env: SWARMCODE_DESKTOP_SIGNED).",
    ),
    Flag.optional,
  ),
  verbose: Flag.Boolean("verbose").pipe(
    Flag.withDescription("Stream subprocess stdout (env: SWARMCODE_DESKTOP_VERBOSE)."),
    Flag.optional,
  ),
  mockUpdates: Flag.Boolean("mock-updates").pipe(
    Flag.withDescription("Enable mock updates (env: SWARMCODE_DESKTOP_MOCK_UPDATES)."),
    Flag.optional,
  ),
  mockUpdateServerPort: Flag.Int("mock-update-server-port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Mock update server port (env: SWARMCODE_DESKTOP_MOCK_UPDATE_SERVER_PORT)."),
    Flag.optional,
  ),
  wslRuntime: Flag.String("wsl-runtime").pipe(
    Flag.withDescription(
      "Path to the Linux CLI release archive (swarm-code-<version>-linux-x64.tar.gz) to embed as the WSL runtime of a Windows build (env: SWARMCODE_DESKTOP_WSL_RUNTIME).",
    ),
    Flag.optional,
  ),
}).pipe(
  Command.withDescription("Build a desktop artifact for Swarm Code."),
