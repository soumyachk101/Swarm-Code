...
const smokeCliArchive = Effect.fn("smokeCliArchive")(function* (input: {
  ...
  const spawner = yield* ChildProcessSpawner.ChildProcessSpawner;
  const scratch = yield* fs.makeTempDirectoryScoped({ prefix: "swarm-code-cli-smoke-" });
  ...
  const executable = path.join(contentDir, platform === "win32" ? "swarmcode.exe" : "swarmcode");
