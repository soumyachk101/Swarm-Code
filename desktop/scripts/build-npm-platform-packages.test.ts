...
const makeFakeArchives = Effect.fn("test.makeFakeArchives")(function* () {
  ...
  for (const key of KEYS) {
    ...
    yield* fs.writeFileString(
      path.join(contentDir, "swarm-code"),
      `#!/bin/sh\necho "stub ${key} $*"\nexit 7\n`,
    );
    yield* fs.chmod(path.join(contentDir, "swarm-code"), 0o755);
  ...
  it.effect("builds platform packages and a launcher that execs the installed one", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(
        ...
        ["@swarmcode/swarm-code-darwin-arm64", "@swarmcode/swarm-code-linux-x64", "swarm-code"],
      );
      for (const output of outputs) {
        assert.isTrue(yield* fs.exists(output.tarball), output.tarball);
      }

      const linuxDir = path.join(fixture.outputDir, "@swarmcode/swarm-code-linux-x64");
      const linuxManifest = yield* decodeManifest(
        yield* fs.readFileString(path.join(linuxDir, "package.json")),
      );
      assert.equal(linuxManifest.name, "@swarmcode/swarm-code-linux-x64");
      ...
      assert.deepStrictEqual(launcherManifest.optionalDependencies, {
        "@swarmcode/swarm-code-darwin-arm64": VERSION,
        "@swarmcode/swarm-code-linux-x64": VERSION,
      });
      ...
      assert.deepStrictEqual(outputEntries.sort(), ["@swarmcode", "swarm-code", "swarm-code.tgz"]);
      ...
      const listing = yield* run(
        "tar",
        ["-tzvf", path.join(fixture.outputDir, "@swarmcode/swarm-code-linux-x64.tgz")],
      ...
      if (KEYS.some((key) => key === `${hostPlatform}-${hostArch}`)) {
        ...
        const unpack = yield* run(
          "tar",
          ["-xf", path.join(fixture.outputDir, "swarm-code.tgz"), "-C", installedLauncher],
          {
            cwd: fixture.root,
          },
        );
      ...
      assert.include(unsupported.stderr, "https://github.com/soumyachk101/Swarm-Code/releases");
    }),
  );
});
