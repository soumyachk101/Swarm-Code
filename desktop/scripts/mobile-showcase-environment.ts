// @effect-diagnostics nodeBuiltinImport:off globalTimers:off globalDate:off - This host-side fixture creates an isolated local Swarm Code environment.
...
export const SHOWCASE_PROJECT_ID = "swarm-code";
...
const SHOWCASE_TERMINAL_PROMPT =
  "\u001b[1;32m→\u001b[0m \u001b[1;36mSwarmCode\u001b[0m \u001b[1;34mgit:(\u001b[1;31mfeat/remote-command-center\u001b[1;34m)\u001b[0m \u001b[1;33m✗\u001b[0m ";
...
export const SHOWCASE_TERMINAL_BUFFER = [
  ...
  "  \u001b[32m→\u001b[0m  \u001b[2mProject:\u001b[0m  \u001b[1mSwarmCode\u001b[0m \u001b[2m— ~/Code/SwarmCode\u001b[0m",
  "",
...
const PROJECT_FAVICONS = {
  SwarmCode: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 128 128">
...
export const SHOWCASE_PROJECTS = [
  {
    id: "swarm-code",
    title: "Swarm Code",
    directory: "swarm-code",
    repositoryUrl: "https://github.com/pingdotgg/SwarmCode.git",
    favicon: PROJECT_FAVICONS.SwarmCode,
  },
  {
    id: "react",
    title: "React",
...
async function seedSwarmCodeWorkspace(workspaceRoot: string): Promise<void> {
  ...
  await NodeFSP.writeFile(
    ...
    `${JSON.stringify({ name: "swarm-code", private: true, scripts: { test: "vp test" } }, null, 2)}\n`,
  );
  await NodeFSP.writeFile(NodePath.join(workspaceRoot, "favicon.svg"), PROJECT_FAVICONS.SwarmCode);
  await NodeFSP.writeFile(
...
async function seedCompanionWorkspace(input: {
  ...
  await NodeFSP.writeFile(
    ...
    `# ${input.title}\n\nSeeded by the Swarm Code mobile screenshot harness.\n`,
  );
  await initializeRepository({
...
export async function seedShowcaseEnvironment(input: {
  ...
  if (primaryProject.id === SHOWCASE_PROJECT_ID) {
    await seedSwarmCodeWorkspace(workspaceRoot);
  }
  await Promise.all(
    projects
