...
describe.skipIf(HostProcessPlatform.defaultValue() !== "linux")("installer terminal", () => {
  it.each([false, true])(
    ...
    async (fail) => {
      const root = await NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "swarm-code-install-progress-"));
      const version = "1.2.3";
      const stem = `swarm-code-${version}-linux-${HostProcessArchitecture.defaultValue()}`;
      ...
      const child = NodeChildProcess.spawn("script", ["-qec", `sh '${installer}'`, "/dev/null"], {
        env: {
          ...
          TERM: "xterm",
          NO_COLOR: "1",
          SWARMCODE_VERSION: version,
          SWARMCODE_HOME: NodePath.join(root, "home"),
          SWARMCODE_INSTALL_BIN_DIR: NodePath.join(root, "bin"),
          SWARMCODE_RELEASE_BASE_URL: `http://127.0.0.1:${address.port}`,
        },
      ...
      try {
        ...
        if (fail) {
          ...
          expect(output).not.toContain("Installed Swarm Code");
          ...
          expect(output).toContain("Installed Swarm Code 1.2.3");
          expect(
