 ...
 * archive: the `swarmcode` binary, the web client, the resource monitor, and a
 ...
 * archive stem):
 *
 *   swarmcode | swarmcode.exe   the single-executable
...
export function cliArchiveStem(version: string, platform: BuildPlatform, arch: BuildArch): string {
  return `swarmcode-${version}-${cliArchivePlatformKey(platform, arch)}`;
}
...
const MacSigningConfig = Config.all({
  identity: Config.String("SWARMCODE_CLI_MAC_SIGN_IDENTITY").pipe(Config.option),
...
const signMacArchiveContents = Effect.fn("signMacArchiveContents")(function* (input: {
  ...
  const libraries = (yield* fs.readDirectory(input.contentDir, { recursive: true }))
    .filter(
      (entry) =>
        ...
        entry.endsWith("swarmcode-resource-monitor"),
    )
  ...
  if (identity === "-") {
    yield* Effect.log("[cli-archive] Signed ad hoc (no SWARMCODE_CLI_MAC_SIGN_IDENTITY).");
    return;
  }
  ...
  const notarizeZip = path.join(path.dirname(input.executablePath), ".notarize-swarmcode.zip");
  yield* runCommand(
  ...
  yield* Effect.log("[cli-archive] Notarized swarmcode.");
});
...
/** Signs swarmcode.exe through the same Azure Trusted Signing setup the installer uses. */
const signWindowsExecutable = Effect.fn("signWindowsExecutable")(function* (
  ...
  yield* runCommand(
    ...
    "Invoke-TrustedSigning swarmcode.exe",
  );
  yield* Effect.log("[cli-archive] Signed swarmcode.exe (Azure Trusted Signing).");
});
...
const buildCliArchive = Effect.fn("buildCliArchive")(function* (input: {
  ...
  const executableName = input.platform === "win" ? "swarmcode.exe" : "swarmcode";
  // tsdown suffixes cross-built executables with their target (swarmcode-darwin-x64);
  // a host build is plain swarmcode. Prefer the exact target when both exist.
  const targetKey = `${input.platform === "mac" ? "darwin" : input.platform}-${input.arch}`;
  const targetExecutable = path.join(
    serverDir,
    "dist-exe",
    `swarmcode-${targetKey}${input.platform === "win" ? ".exe" : ""}`,
  );
  ...
  yield* requireInput(path.join(webClient, "index.html"), "Run `vp run --filter swarmcode build` first.");
  yield* requireInput(
    resourceMonitorDir,
  ...
  const stageRoot = yield* fs.makeTempDirectoryScoped({ prefix: "swarmcode-cli-archive-" });
...
).pipe(Command.withDescription("Package the swarmcode single-executable into a per-platform archive."));
