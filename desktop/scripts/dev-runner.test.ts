...
} from "@swarmcode/shared/hostProcess";
...
const devServerInput = {
  mode: "dev:server",
  swarmCodeHome: "/tmp/swarm-code-dev-runner",
  browser: undefined,
...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  ...
  describe("resolveOffset", () => {
    it.effect("uses explicit SWARMCODE_PORT_OFFSET when provided", () =>
      Effect.gen(function* () {
        const result = yield* resolveOffset({ portOffset: 12, devInstance: undefined });
        assert.deepStrictEqual(result, {
          offset: 12,
          source: "SWARMCODE_PORT_OFFSET=12",
        });
      }),
    );
    ...
    it.effect("returns structured context for a negative port offset", () =>
      Effect.gen(function* () {
        ...
        assert.equal(error.configKey, "SWARMCODE_PORT_OFFSET");
  ...
  describe("createDevRunnerEnv", () => {
    it.effect("forwards the reusable auth token to web dev and removes it for desktop", () =>
      Effect.gen(function* () {
        const input = {
          baseEnv: { SWARMCODE_DEV_AUTH_TOKEN: "reusable-dev-auth-token-that-is-long-enough" },
          serverOffset: 0,
          webOffset: 0,
          swarmCodeHome: undefined,
          browser: undefined,
          autoBootstrapProjectFromCwd: undefined,
          logWebSocketEvents: undefined,
          host: undefined,
        ...
        assert.equal(web.SWARMCODE_DEV_AUTH_TOKEN, input.baseEnv.SWARMCODE_DEV_AUTH_TOKEN);
        assert.equal(desktop.SWARMCODE_DEV_AUTH_TOKEN, undefined);
      }),
    );
    it.effect("leaves the shared home implicit and disables browser auto-open", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.SWARMCODE_HOME, undefined);
        assert.equal(env.SWARMCODE_NO_BROWSER, "1");
      }),
    );
    ...
    it.effect("allows browser auto-open to be explicitly enabled", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.SWARMCODE_NO_BROWSER, "0");
      }),
    );
    ...
    it.effect("requires the browser flag even when the environment enables auto-open", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: { SWARMCODE_NO_BROWSER: "0" },
          serverOffset: 0,
          webOffset: 0,
          swarmCodeHome: undefined,
          browser: false,
    ...
    it.effect("supports explicit typed overrides", () =>
      Effect.gen(function* () {
        ...
        const env = yield* createDevRunnerEnv({
          ...
          serverOffset: 0,
          webOffset: 0,
          swarmCodeHome: "/tmp/custom-swarm-code",
          browser: false,
        ...
        assert.equal(env.SWARMCODE_HOME, path.resolve("/tmp/custom-swarm-code"));
        assert.equal(env.SWARMCODE_PORT, "4222");
        assert.equal(env.VITE_HTTP_URL, "http://localhost:4222");
        assert.equal(env.VITE_WS_URL, "ws://localhost:4222");
        assert.equal(env.SWARMCODE_NO_BROWSER, "1");
        assert.equal(env.SWARMCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD, "0");
        assert.equal(env.SWARMCODE_LOG_WS_EVENTS, "1");
        assert.equal(env.SWARMCODE_HOST, "0.0.0.0");
    ...
    it.effect("strips inherited service-launcher context", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {
            SWARMCODE_SERVICE_LAUNCHER_CONTEXT: '{"childVersion":"9.9.9"}',
            SWARM_CODE_BOOT_SERVICE_UNIT: "swarm-code.service",
          },
          serverOffset: 0,
          webOffset: 0,
          swarmCodeHome: undefined,
        ...
        assert.equal(env.SWARMCODE_SERVICE_LAUNCHER_CONTEXT, undefined);
        assert.equal(env.SWARM_CODE_BOOT_SERVICE_UNIT, undefined);
      }),
    );
    ...
    it.effect("does not force websocket logging on in dev mode when unset", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {
            SWARMCODE_LOG_WS_EVENTS: "keep-me-out",
          },
          serverOffset: 0,
          webOffset: 0,
          swarmCodeHome: undefined,
        ...
        assert.equal(env.SWARMCODE_MODE, "web");
        assert.equal(env.SWARMCODE_LOG_WS_EVENTS, undefined);
      }),
    );
    ...
    it.effect("forwards explicit websocket logging false without coercing it away", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.SWARMCODE_LOG_WS_EVENTS, "0");
      }),
    );
    ...
    it.effect("uses custom t3Home when provided", () =>
      Effect.gen(function* () {
        ...
        const env = yield* createDevRunnerEnv({
          ...
          serverOffset: 0,
          webOffset: 0,
          swarmCodeHome: "/tmp/my-swarm-code",
          browser: undefined,
          autoBootstrapProjectFromCwd: undefined,
          logWebSocketEvents: undefined,
          host: undefined,
        ...
        assert.equal(env.SWARMCODE_HOME, path.resolve("/tmp/my-swarm-code"));
      }),
    );
    ...
    it.effect("pins desktop dev to a stable backend port and websocket url", () =>
      Effect.gen(function* () {
        ...
        const env = yield* createDevRunnerEnv({
          mode: "dev:desktop",
          baseEnv: {
            SWARMCODE_PORT: "13773",
            SWARMCODE_MODE: "web",
            SWARMCODE_NO_BROWSER: "0",
            SWARMCODE_HOST: "0.0.0.0",
        ...
        assert.equal(env.SWARMCODE_MODE, undefined);
        assert.equal(env.SWARMCODE_NO_BROWSER, undefined);
        assert.equal(env.SWARMCODE_HOST, undefined);
    ...
    it.effect("defaults dev server mode to the higher backend port range", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.SWARMCODE_PORT, "13773");
        assert.equal(env.PORT, "5733");
      }),
    );
    ...
    for (const mode of ["dev", "dev:web"] as const) {
      it.effect(`leaves the client backend URLs unset in ${mode} mode`, () =>
        Effect.gen(function* () {
          ...
          assert.equal(env.SWARMCODE_SINGLE_ORIGIN_DEV, "1");
        }),
      );
    }
    ...
    it.effect("clears the single-origin marker in dev:desktop mode", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev:desktop",
          baseEnv: { SWARMCODE_SINGLE_ORIGIN_DEV: "1" },
          serverOffset: 0,
          webOffset: 0,
          swarmCodeHome: undefined,
          browser: undefined,
        ...
        assert.equal(env.SWARMCODE_SINGLE_ORIGIN_DEV, undefined);
    ...
    // --host configures the *backend* (SWARMCODE_HOST). It must not become Vite's
  ...
  describe("devPortProbeHosts", () => {
    ...
    // actually bind, so --host/SWARMCODE_HOST has to be probed as well.
    ...
    // Only the backend honours --host/SWARMCODE_HOST. Vite reads HOST (set for
  ...
  describe("runDevRunnerWithInput", () => {
    it.effect("preserves invalid configuration as the exact cause", () =>
      Effect.gen(function* () {
          ...
          Effect.provide(
            Layer.merge(
              ...
              ConfigProvider.layer(
                ConfigProvider.fromEnv({ env: { SWARMCODE_PORT_OFFSET: "not-an-integer" } }),
              ),
            ),
          ),
          Effect.flip,
        ...
        assert.deepStrictEqual(error.configKeys, ["SWARMCODE_PORT_OFFSET", "SWARMCODE_DEV_INSTANCE"]);
    ...
    // Base-dir precedence (--home-dir > worktree .t3 > ambient SWARMCODE_HOME)
    ...
    it.effect("ignores an ambient SWARMCODE_HOME when no home is resolved", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: { SWARMCODE_HOME: "/home/user/.swarm-code" },
          serverOffset: 0,
          webOffset: 0,
          swarmCodeHome: undefined,
          browser: undefined,
    ...
    // defaults: an explicit SWARMCODE_BUNDLED_DEV (even "0") must pass through.
    describe("--share bundled dev default", () => {
          ...
          yield* runDevRunnerWithInput({
            ...
            Effect.provideService(
              ...
              input.ambientBundledDev === undefined
                ? {}
                : { SWARMCODE_BUNDLED_DEV: input.ambientBundledDev },
            ),
          );
      ...
      it.effect("defaults SWARMCODE_BUNDLED_DEV=1 for a shared run", () =>
        Effect.gen(function* () {
          const env = yield* shareSpawnedEnv({ ambientBundledDev: undefined });
          assert.equal(env?.SWARMCODE_BUNDLED_DEV, "1");
        }),
      );

      it.effect("keeps an explicit SWARMCODE_BUNDLED_DEV=0 opt-out", () =>
        Effect.gen(function* () {
          const env = yield* shareSpawnedEnv({ ambientBundledDev: "0" });
          assert.equal(env?.SWARMCODE_BUNDLED_DEV, "0");
        }),
      );

      it.effect("leaves SWARMCODE_BUNDLED_DEV unset without --share", () =>
        Effect.gen(function* () {
          let captured: Record<string, string | undefined> | undefined;
          const spawnerLayer = Layer.succeed(
            ChildProcessSpawner.ChildProcessSpawner,
          ...
          assert.equal(captured?.SWARMCODE_BUNDLED_DEV, undefined);
        }),
      );
    });
    ...
    describe("swarm-code home precedence", () => {
      const makeWorktree = Effect.acquireRelease(
        Effect.sync(() => {
          const root = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "swarm-code-devrunner-"));
      ...
      const spawnedHome = (input: {
        ...
        Effect.gen(function* () {
          ...
          yield* runDevRunnerWithInput({ ...devServerInput, swarmCodeHome: input.t3Home }).pipe(
            Effect.provide(Layer.mergeAll(emptyConfigLayer, netServiceLayer, spawnerLayer)),
            Effect.provideService(HostProcessPlatform, "linux"),
            Effect.provideService(HostProcessWorkingDirectory, input.cwd),
            Effect.provideService(
              HostProcessEnvironment,
              input.ambientHome === undefined ? {} : { SWARMCODE_HOME: input.ambientHome },
            ),
          );

          return captured?.SWARMCODE_HOME;
        });
      ...
      it.effect("prefers an explicit --home-dir over the worktree default", () =>
        Effect.gen(function* () {
          ...
          const home = yield* spawnedHome({
            swarmCodeHome: "/tmp/explicit-home",
            cwd: root,
            ambientHome: "/home/user/.swarm-code",
          });
      ...
      it.effect("prefers the worktree .t3 over an ambient SWARMCODE_HOME", () =>
        Effect.gen(function* () {
          const path = yield* Path.Path;
          const root = yield* makeWorktree;
          const home = yield* spawnedHome({
      ...
      it.effect("falls back to an ambient SWARMCODE_HOME outside a worktree", () =>
        Effect.gen(function* () {
          const path = yield* Path.Path;
          const home = yield* spawnedHome({
            swarmCodeHome: undefined,
          ...
          assert.equal(home, path.resolve("/home/user/.swarm-code"));
        }),
      );
