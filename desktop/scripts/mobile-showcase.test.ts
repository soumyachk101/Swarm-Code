...
it("selects an explicit CI Android ABI without changing the local default", () => {
  ...
  assert.throws(() => resolveShowcaseAndroidAbi("mips"), /Unsupported SWARM_CODE_SHOWCASE_ANDROID_ABI/u);
});
...
it("defaults every device to the app's own palette", () => {
  assert.equal(DEFAULT_SHOWCASE_THEME, "swarm-code");
  assert.equal(
