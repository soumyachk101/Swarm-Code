...
const ANDROID_PACKAGE = "com.swarmcode.swarm-code";
const APP_SCHEME = "swarm-code";
const IOS_READY_FILENAME = "SwarmCodeShowcaseReadyScene";
const SERVER_HOST = "0.0.0.0";
const IOS_SIMULATOR_ARCH = NodeProcess.arch === "arm64" ? "arm64" : "x86_64";
const IOS_APP_PATH = NodePath.join(
  MOBILE_ROOT,
  ".showcase/ios-derived-data/Build/Products/Debug-iphonesimulator/swarm-code.app",
);
...
const MOBILE_BUILD_ENV = {
  ...
  SWARM_CODE_SHOWCASE_CAPTURE_BUILD: "1",
  JAVA_HOME:
...
Usage:
  pnpm --filter @swarmcode/mobile screenshots [options]
...
async function buildIos(): Promise<string> {
  ...
  await runCommand(
    ...
    [
      "-workspace",
      NodePath.join(MOBILE_ROOT, "ios/swarm-code.xcworkspace"),
      "-scheme",
      "swarm-code",
      "-configuration",
      "Debug",
...
async function captureIos(
  ...
  const scenePath = NodePath.join(
    ...
    "Library/Caches/SwarmCodeShowcaseScene",
  );
  const readyPath = NodePath.join(
    await iosAppContainer(simulator.udid),
    "Library/Caches",
...
async function waitForAndroidShowcaseScene(
  ...
  while (Date.now() < deadline) {
    const readyScene = await adbOutput(serial, [
      ...
      ANDROID_PACKAGE,
      "cat",
      "files/swarm-code-showcase-ready",
    ]).catch(() => "");
    if (readyScene.trim() === scene) return;
    await delay(500);
  }
...
async function writeAndroidShowcaseScene(serial: string, scene: ShowcaseScene): Promise<void> {
  await runAdb(serial, [
    "shell",
    `run-as ${ANDROID_PACKAGE} sh -c 'mkdir -p files && rm -f files/swarm-code-showcase-ready && printf %s ${scene} > files/swarm-code-showcase-scene'`,
  ]);
}
...
async function main(): Promise<void> {
  ...
  const showcaseRootDir = await NodeFSP.mkdtemp(
    NodePath.join(NodeOS.tmpdir(), "swarm-code-mobile-showcase-"),
  );
