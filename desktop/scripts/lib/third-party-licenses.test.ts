...
async function createFixture(): Promise<{
  ...
  const root = await NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "swarm-code-licenses-"));
...
describe("third-party license generation", () => {
  it("keeps the GhosttyKit notice pinned to the vendored framework revision", async () => {
    ...
    expect(config).toContain(
      "https://github.com/Yash-Singh1/ghostty/tree/swarm-code/custom-io-ordered-feed",
    );
  });
          ...
          generatedNotices: [
            {
              ...
              preamble: ["Adapted for Swarm Code."],
            },
          ],
          bundles: ["assets", "web"],
        },
