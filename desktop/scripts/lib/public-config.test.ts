...
describe("loadRepoEnv", () => {
  it("does not project cloud configuration for an unconfigured clone", () => {
    ...
    expect(env.SWARMCODE_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.SWARMCODE_CLERK_CLI_OAUTH_CLIENT_ID).toBeUndefined();
    expect(env.VITE_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.SWARMCODE_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.VITE_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.EXPO_PUBLIC_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.SWARMCODE_RELAY_URL).toBeUndefined();
    expect(env.VITE_SWARMCODE_RELAY_URL).toBeUndefined();
    expect(env.SWARMCODE_MOBILE_OTLP_TRACES_URL).toBeUndefined();
    expect(env.SWARMCODE_MOBILE_OTLP_TRACES_DATASET).toBeUndefined();
    expect(env.SWARMCODE_MOBILE_OTLP_TRACES_TOKEN).toBeUndefined();
    ...
    expect(env.SWARMCODE_RELAY_CLIENT_OTLP_TRACES_URL).toBeUndefined();
    expect(env.SWARMCODE_RELAY_CLIENT_OTLP_TRACES_DATASET).toBeUndefined();
    expect(env.SWARMCODE_RELAY_CLIENT_OTLP_TRACES_TOKEN).toBeUndefined();
  ...
  it("applies process, root local, and root precedence in that order", () => {
    ...
    NodeFS.writeFileSync(
      ...
      "SWARMCODE_CLERK_PUBLISHABLE_KEY=pk_root\nSWARMCODE_CLERK_JWT_TEMPLATE=template_root\nSWARMCODE_CLERK_CLI_OAUTH_CLIENT_ID=oauth_root\nSWARMCODE_RELAY_URL=https://root.example.test\n",
    );
    NodeFS.writeFileSync(
      NodePath.join(repoRoot, ".env.local"),
      "SWARMCODE_CLERK_PUBLISHABLE_KEY=pk_local\nSWARMCODE_CLERK_JWT_TEMPLATE=template_local\nSWARMCODE_CLERK_CLI_OAUTH_CLIENT_ID=oauth_local\nSWARMCODE_RELAY_URL=https://local.example.test\n",
    );

    expect(loadRepoEnv({ baseEnv: {}, repoRoot }).SWARMCODE_RELAY_URL).toBe(
      "https://local.example.test",
    );
    expect(
      loadRepoEnv({
        baseEnv: {
          SWARMCODE_CLERK_PUBLISHABLE_KEY: "pk_ci",
          SWARMCODE_CLERK_JWT_TEMPLATE: "template_ci",
          SWARMCODE_CLERK_CLI_OAUTH_CLIENT_ID: "oauth_ci",
          SWARMCODE_RELAY_URL: "https://ci.example.test",
        },
        repoRoot,
      }),
    ).toMatchObject({
      ...
      SWARMCODE_RELAY_URL: "https://ci.example.test",
      VITE_SWARMCODE_RELAY_URL: "https://ci.example.test",
    });
  });
  ...
  it("accepts legacy framework aliases as root overrides", () => {
    expect(
      resolvePublicConfig({
        ...
        SWARMCODE_CLERK_CLI_OAUTH_CLIENT_ID: "oauth_canonical",
        VITE_SWARMCODE_RELAY_URL: "https://legacy.example.test",
  ...
  it("projects canonical relay client tracing values to web build aliases", () => {
    expect(
      loadRepoEnv({
        baseEnv: {
          SWARMCODE_RELAY_CLIENT_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
          SWARMCODE_RELAY_CLIENT_OTLP_TRACES_DATASET: "relay-client-traces",
          SWARMCODE_RELAY_CLIENT_OTLP_TRACES_TOKEN: "relay-client-token",
        },
  ...
  it("projects canonical mobile tracing values to Expo public aliases", () => {
    expect(
      loadRepoEnv({
        baseEnv: {
          SWARMCODE_RELAY_URL: "https://relay.example.test",
          SWARMCODE_MOBILE_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
          SWARMCODE_MOBILE_OTLP_TRACES_DATASET: "mobile-traces",
          SWARMCODE_MOBILE_OTLP_TRACES_TOKEN: "mobile-token",
        },
        repoRoot: makeTemporaryDirectory(),
      }),
    ).toEqual({
      SWARMCODE_RELAY_URL: "https://relay.example.test",
      VITE_SWARMCODE_RELAY_URL: "https://relay.example.test",
      SWARMCODE_MOBILE_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
      SWARMCODE_MOBILE_OTLP_TRACES_DATASET: "mobile-traces",
      SWARMCODE_MOBILE_OTLP_TRACES_TOKEN: "mobile-token",
      EXPO_PUBLIC_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
...
function makeTemporaryDirectory() {
  const directory = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "swarm-code-public-config-"));
