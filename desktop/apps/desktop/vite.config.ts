...
const shouldLaunchElectronAfterPack = process.env.SWARMCODE_DESKTOP_DEV === "1";
  ...
  run: {
    tasks: {
      build: {
        ...
        dependsOn: ["swarmcode#build"],
        cache: false,
      },
      dev: {
        command:
          "node scripts/build-browser-secret.mjs && node scripts/build-preview-annotation-css.mjs && cross-env SWARMCODE_DESKTOP_DEV=1 vp pack --watch",
        dependsOn: ["swarmcode#build"],
        cache: false,
      },
      "dev:bundle": {
