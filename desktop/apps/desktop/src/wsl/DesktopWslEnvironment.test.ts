...
const SERVER_ENTRY_SOURCE = '#!/bin/sh\necho "swarm-code wsl runtime test server 0.0.0"\n';
...
describe("WSL runtime cache", () => {
  ...
  it("installs through a temporary directory and only reuses valid completed caches", () => {
    const script = buildWslRuntimeInstallScript(
      "/mnt/c/Program Files/Swarm Code/wsl-runtime.tar.gz",
      "1.2.3-x64",
      "b".repeat(64),
    );

    expect(script).toContain('runtime_parent="$HOME/.swarm-code/wsl-runtime"');
    ...
    expect(script).toContain(
      "tar -xzf '/mnt/c/Program Files/Swarm Code/wsl-runtime.tar.gz' -C \"$runtime_tmp\" --strip-components=1",
    );
  ...
  it("verifies the archive digest before extracting, and only on a cache miss", () => {
    ...
    expect(script).toContain(
      "archive_sha=$(sha256sum '/mnt/c/Program Files/Swarm Code/wsl-runtime.tar.gz' | cut -d ' ' -f 1)",
    );
  ...
  it("re-hashes the executable against the digest the install recorded", () => {
    ...
    expect(script).toContain(
      `printf '%s\\n' "$installed_entry_digest" > "$runtime_tmp/.swarm-code-wsl-runtime-ready"`,
    );
    ...
    const markerWritten = script.indexOf('> "$runtime_tmp/.swarm-code-wsl-runtime-ready"');
  ...
  it("parses only absolute Linux runtime paths", () => {
    expect(parseWslRuntimeRoot("runtimeRoot:/home/josh/.swarm-code/wsl-runtime/1.2.3-x64\n")).toBe(
      "/home/josh/.swarm-code/wsl-runtime/1.2.3-x64",
    );
  ...
  it("prunes completed runtimes except the current and newest previous cache", () => {
    ...
    expect(script).toContain('[ -f "$candidate/.swarm-code-wsl-runtime-ready" ] || continue');
  ...
  it("invalidates a cache by dropping its ready marker, not the tree", () => {
    ...
    expect(script).toContain('rm -f "$HOME/.swarm-code/wsl-runtime/1.2.3_x64/.swarm-code-wsl-runtime-ready"');
...
describe.skipIf(posixShellRunner === null)("WSL runtime install script (executed)", () => {
  ...
  const createFixture = () => {
    ...
    return {
      ...
      archiveSha,
      runtimeId,
      runtimeParent: `${work}/home/.swarm-code/wsl-runtime`,
      runtimeRoot: `${work}/home/.swarm-code/wsl-runtime/${runtimeId}`,
      serverEntry: `${work}/home/.swarm-code/wsl-runtime/${runtimeId}/t3`,
      installScript,
  ...
  it("extracts once when two installs race for the same cache", () => {
    ...
    const raced = runShell(
      [
        ...
        'PATH="$work/bin:$PATH"',
        "export PATH",
        `cat > "$work/install.sh" <<'SWARMCODE_INSTALL_SCRIPT'`,
        fixture.installScript(),
        "SWARMCODE_INSTALL_SCRIPT",
  ...
  it("leases a warm cache across the prepare-to-spawn handoff", () => {
    ...
    const selected = runShell(
      [
        ...
        'printf ready > "$runtime_parent/sha256-current/.swarm-code-wsl-runtime-ready"',
        'printf ready > "$runtime_parent/sha256-previous/.swarm-code-wsl-runtime-ready"',
        `touch -d "10 minutes ago" ${sh(fixture.runtimeRoot)}`,
        'touch -d "1 minute ago" "$runtime_parent/sha256-previous"',
        `cat > ${sh(`${fixture.work}/select.sh`)} <<'SWARMCODE_SELECT_SCRIPT'`,
        fixture.installScript(),
        "SWARMCODE_SELECT_SCRIPT",
  ...
  it("removes an aged stale tree after replacing an active unready cache", () => {
    ...
    const result = runShell(
      [
        ...
        `runtime_parent=${sh(fixture.runtimeParent)}`,
        'rm "$runtime_root/.swarm-code-wsl-runtime-ready"',
  ...
  it("prunes old and markerless caches without touching retained, active, locked, or unrelated roots", () => {
    const result = runShell(
      [
        ...
        "work=$(mktemp -d)",
        'home="$work/home"',
        'runtime_parent="$home/.swarm-code/wsl-runtime"',
        'mkdir -p "$runtime_parent"',
        'make_ready() { mkdir -p "$runtime_parent/$1"; printf ready > "$runtime_parent/$1/.swarm-code-wsl-runtime-ready"; }',
