...
import { buildRemoteNodeEnvScript } from "@swarmcode/ssh/tunnel";
...
>()("@swarmcode/desktop/wsl/DesktopWslEnvironment") {}
...
// that install wrote.
const WSL_RUNTIME_READY_MARKER = ".swarm-code-wsl-runtime-ready";
...
export const buildWslRuntimeInvalidateScript = (runtimeId: string): string => {
  ...
  return [
    "set -eu",
    `rm -f "$HOME/.swarm-code/wsl-runtime/${safeRuntimeId}/${WSL_RUNTIME_READY_MARKER}"`,
  ].join("\n");
};
...
const formatNodePtyProbeFailureReason = (exitCode: number): string | null =>
  exitCode === NODE_PTY_BINARY_MISSING_EXIT_CODE
    ? "WSL support is missing from this Swarm Code build: the packaged Linux node-pty binary was not included. Install a build that includes WSL support."
    : null;
