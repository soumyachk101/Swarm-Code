...
function makeEnvironmentLayer(baseDir: string, env: Record<string, string | undefined> = {}) {
  return DesktopEnvironment.layer({
    ...
    Layer.provide(
      Layer.mergeAll(NodeServices.layer, DesktopConfig.layerTest({ SWARMCODE_HOME: baseDir, ...env })),
    ),
  );
}
...
function makeLayer(input: {
  ...
  const env = { SWARMCODE_HOME: input.baseDir, ...input.env };
...
const withHarness = <A, E, R>(
  ...
  Effect.gen(function* () {
    ...
    const baseDir = yield* fileSystem.makeTempDirectoryScoped({
      prefix: "swarm-code-desktop-server-exposure-test-",
    });
...
describe("DesktopServerExposure", () => {
  ...
  it.effect("preserves explicit Tailscale exposure overrides", () =>
    withHarness(
      ...
      {
        SWARMCODE_DESKTOP_LAN_HOST: "100.90.1.2",
        SWARMCODE_DESKTOP_HTTPS_ENDPOINTS: "https://public.example.test",
      },
    ),
  );
