...
>()("@swarmcode/desktop/backend/DesktopBackendConfiguration") {}
...
const DESKTOP_BACKEND_ENV_NAMES = [
  "SWARMCODE_PORT",
  "SWARMCODE_MODE",
  "SWARMCODE_NO_BROWSER",
  "SWARMCODE_HOST",
  "SWARMCODE_DESKTOP_WS_URL",
  "SWARMCODE_DESKTOP_LAN_ACCESS",
  "SWARMCODE_DESKTOP_LAN_HOST",
  "SWARMCODE_DESKTOP_HTTPS_ENDPOINTS",
  "SWARMCODE_TAILSCALE_SERVE",
  "SWARMCODE_TAILSCALE_SERVE_PORT",
] as const;
...
const WSL_FORWARDED_ENV_NAMES = [
  "OPENAI_API_KEY",
  "ANTHROPIC_API_KEY",
  "SWARMCODE_OTLP_HEADERS",
  "SWARMCODE_OTLP_PROTOCOL",
] as const;
...
function resourceMonitorBinaryName(platform: NodeJS.Platform): string {
  return platform === "win32" ? "swarm-code-resource-monitor.exe" : "swarm-code-resource-monitor";
}
...
// self-contained `swarm-code` executable (Node inside); the mounted server tree is a
    ...
    if (runtime.ok) {
      ...
      if (stagedProbe.ok) {
        ...
        return {
          ...
          runtime: { kind: "executable", entryPath: `${runtime.linuxAppRoot}/swarm-code` },
...
const resolvePrimaryStartConfig = Effect.fn("desktop.backendConfiguration.resolvePrimary")(
  ...
  > {
    ...
    return {
      ...
      // Primary wants process.env (PATH, dev-runner's SWARMCODE_HOME, etc.).
      extendEnv: true,
      bootstrap,
...
> {
  ...
  // Build an explicit copy of process.env minus SWARMCODE_HOME (dev-runner
  // exports the Windows-side base dir for the primary; if it leaks into
  // the WSL backend the Linux side ends up sharing C:\Users\...\.swarm-code via
  // /mnt/c, which means both backends read/write the same database and
  // their env-ids collide).
  const parentEnvWithoutSwarmCodeHome: Record<string, string | undefined> = {};
  for (const [key, value] of Object.entries(process.env)) {
    if (key === "SWARMCODE_HOME") continue;
    parentEnvWithoutSwarmCodeHome[key] = value;
  }
  const wslEnv = mergeWslEnv(parentEnvWithoutSwarmCodeHome.WSLENV, forwardedEnvNames);
  ...
  const baseConfig = {
    ...
    cwd: environment.backendCwd,
    env: {
      ...parentEnvWithoutSwarmCodeHome,
      ...backendChildEnvPatch(),
      ...forwardedEnv,
      ...(wslEnv !== undefined ? { WSLENV: wslEnv } : {}),
    },
    // env is already a complete process.env minus SWARMCODE_HOME; pass it
