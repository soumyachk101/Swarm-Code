    ...
    Layer.provide(
      Layer.mergeAll(
        ...
        DesktopConfig.layerTest({
          SWARMCODE_HOME: baseDir,
          SWARMCODE_PORT: "9999",
          SWARMCODE_MODE: "desktop",
          SWARMCODE_DESKTOP_LAN_HOST: "192.168.1.50",
          VITE_DEV_SERVER_URL: options?.devServerUrl,
          SWARMCODE_OTLP_TRACES_URL: options?.otlpTracesUrl,
          SWARMCODE_OTLP_METRICS_URL: options?.otlpMetricsUrl,
          SWARMCODE_OTLP_LOGS_URL: options?.otlpLogsUrl,
        }),
      ),
    ),
  );
...
const withHarness = <A, E, R>(
  ...
  Effect.gen(function* () {
    ...
    const baseDir = yield* fileSystem.makeTempDirectoryScoped({
      prefix: "swarm-code-desktop-backend-config-test-",
    });
...
describe("DesktopBackendConfiguration", () => {
  it.effect("resolvePrimary produces a stable scoped bootstrap token", () =>
    withHarness(
      Effect.gen(function* () {
        ...
        assert.isUndefined(first.env.SWARMCODE_PORT);
        assert.isUndefined(first.env.SWARMCODE_MODE);
        assert.isUndefined(first.env.SWARMCODE_DESKTOP_LAN_HOST);
  ...
  it.effect("resolveWsl launches a packaged backend from the WSL-local runtime cache", () => {
    ...
    const linuxAppRoot = "/home/test/.swarm-code/wsl-runtime/1.2.3-x64";
    ...
    return withPackagedWslHarness(
      ...
      ({ hashPath, mountedAppRoot }) =>
        Effect.gen(function* () {
          ...
          assert.include(first.args, `/runtime/sha256-${firstHash}/swarm-code`);
          assert.equal(second.wslRuntimeId, observedRuntimeIds[1]);
          assert.include(second.args, `/runtime/sha256-${secondHash}/swarm-code`);
  ...
  it.effect("resolveWsl retires a staged runtime whose executable does not start", () => {
    ...
    const stagedAppRoot = `/home/test/.swarm-code/wsl-runtime/sha256-${archiveHash}`;
    const observedProbeRoots: string[] = [];
    const observedNodePtyRoots: string[] = [];
    ...
    return withPackagedWslHarness(
      {
        ...
        wsl: () => ({
          ...
          probeRuntime: (_distro, root) => {
            observedProbeRoots.push(root);
            return { ok: false, reason: `${root}/swarm-code --version failed (exit 127)` };
          },
          ensureNodePty: (_distro, root) => {
            observedNodePtyRoots.push(root);
            return { ok: true, nodePath: "/usr/bin/node", resolvedPath: "/usr/bin:/bin" };
      ...
      ({ mountedAppRoot, mountedEntryPath }) =>
        Effect.gen(function* () {
          ...
          assert.include(config.args, "/usr/bin/node");
          assert.include(config.args, `${mountedAppRoot}/apps/server/dist/bin.mjs`);
          assert.notInclude(config.args, `${stagedAppRoot}/swarm-code`);
          assert.equal(config.entryPath, mountedEntryPath);
          assert.isUndefined(config.wslRuntimeId);
          assert.isTrue(Option.isNone(config.preflightFailure));
  ...
  it.effect("resolveWsl keeps the staged runtime when the mounted tree fails too", () => {
    const stagedAppRoot = "/home/test/.swarm-code/wsl-runtime/cache";
    const invalidatedRuntimeIds: string[] = [];
    return withPackagedWslHarness(
      {
    ...
    return withPackagedWslHarness(
      {
        ...
        wsl: () => ({
          ...
          probeRuntime: () => ({ ok: false, reason: "swarm-code --version failed (exit 1)" }),
          ensureNodePty: () => ({
            ok: false,
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const linuxAppRoot = "/tmp/swarm-code's launch";
  ...
  it.effect("resolveWsl carries environment-configured observability endpoints", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        assert.equal(config.bootstrap.otlpMetricsUrl, "http://127.0.0.1:4318/v1/metrics");
        assert.equal(config.bootstrap.otlpLogsUrl, "http://127.0.0.1:4318/v1/logs");
        assert.notInclude(config.env.WSLENV ?? "", "SWARMCODE_OTLP_LOGS_URL");
      }).pipe(
        Effect.provide(
          DesktopBackendConfiguration.layer.pipe(
            Layer.provideMerge(serverExposureLayer),
  ...
  it.effect("resolveWsl preserves existing WSLENV entries when forwarding backend secrets", () =>
    Effect.gen(function* () {
      ...
      const previousOtlpHeaders = process.env.SWARMCODE_OTLP_HEADERS;
      const previousOtlpProtocol = process.env.SWARMCODE_OTLP_PROTOCOL;
      try {
        process.env.WSLENV = "GOPATH/p:OPENAI_API_KEY/u:EMPTY::AZURE_DEVOPS_EXT_PAT/u";
        process.env.OPENAI_API_KEY = "openai-key";
        process.env.ANTHROPIC_API_KEY = "anthropic-key";
        process.env.SWARMCODE_OTLP_HEADERS = 'authorization="Bearer%20my-token"';
        process.env.SWARMCODE_OTLP_PROTOCOL = "http/protobuf";
        ...
        yield* Effect.gen(function* () {
          ...
          assert.equal(config.env.SWARMCODE_OTLP_PROTOCOL, "http/protobuf");
          ...
          assert.equal(
            config.env.WSLENV,
            "GOPATH/p:OPENAI_API_KEY/u:EMPTY::AZURE_DEVOPS_EXT_PAT/u:ANTHROPIC_API_KEY:SWARMCODE_OTLP_HEADERS:SWARMCODE_OTLP_PROTOCOL",
          );
        }).pipe(
          Effect.provide(
            DesktopBackendConfiguration.layer.pipe(
        ...
        restoreEnv("SWARMCODE_OTLP_HEADERS", previousOtlpHeaders);
        restoreEnv("SWARMCODE_OTLP_PROTOCOL", previousOtlpProtocol);
      }
    }).pipe(Effect.scoped, Effect.provide(NodeServices.layer)),
  );
  ...
  it.effect("prefers the external packaged resource monitor over the copy inside the asar", () =>
    Effect.gen(function* () {
      ...
      const embeddedMonitorPath = `${resourcesPath}/app.asar/apps/desktop/prod-resources/resource-monitor/swarm-code-resource-monitor`;
      const monitorPath = path.join(resourcesPath, "resource-monitor/swarm-code-resource-monitor");
  ...
  it.effect("prefers the release resource monitor when both development builds exist", () =>
    Effect.gen(function* () {
      ...
      const releaseMonitorPath = path.join(
        baseDir,
        "native/resource-monitor/target/release/swarm-code-resource-monitor",
      );
      const debugMonitorPath = path.join(
        baseDir,
        "native/resource-monitor/target/debug/swarm-code-resource-monitor",
      );
  ...
  it("resolvePrimaryLabel is runSync-safe against the real WSL availability probe", async () => {
    ...
    const runtime = ManagedRuntime.make(
      DesktopBackendConfiguration.layer.pipe(
        ...
        Layer.provideMerge(makeEnvironmentLayer("/tmp/swarm-code-wsl-isavailable", { platform: "win32" })),
