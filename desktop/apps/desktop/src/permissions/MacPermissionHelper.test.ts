...
vi.mock("electron", async () => {
  ...
  return {
    app: {
      getPath: () => "/Applications/Swarm Code (Nightly).app/Contents/MacOS/Swarm Code",
    },
...
describe("macAppBundlePath", () => {
  it("resolves bundles with spaces and refuses non-bundle executables", () => {
    expect(macAppBundlePath("/Applications/Swarm Code.app/Contents/MacOS/Swarm Code")).toBe(
      "/Applications/Swarm Code.app",
    );
    expect(macAppBundlePath("/usr/local/bin/electron")).toBeUndefined();
    expect(macAppBundlePath("/Applications/Swarm Code.app/other/MacOS/Swarm Code")).toBeUndefined();
  });
});
it("drags the running app bundle only for the helper's own renderer", async () => {
  ...
  expect(mocks.startDrag).toHaveBeenCalledWith({
    file: "/Applications/Swarm Code (Nightly).app",
    icon: mocks.createFromPath.mock.results[0]!.value.resize(),
  });
  send("finder");
  expect(mocks.showItemInFolder).toHaveBeenCalledWith("/Applications/Swarm Code (Nightly).app");
});
...
it("does not show a helper with a missing packaged icon", async () => {
  ...
  await expect(open()).rejects.toThrow("packaged Swarm Code icon is missing");
  expect(windows).toHaveLength(0);
});
