...
const appId = "com.swarmcode.swarmcode";
const metadata = {
  title: "Editor",
  appName: "Text Editor",
...
beforeEach(async () => {
  ...
  directory = await NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "swarm-code-portal-test-"));
...
it("retains the authenticated connection until the compositor flight has landed", async () => {
  ...
  await feedback.activate("Swarm Code");
  ...
  expect(
    bus.calls
      ...
      .map((call) => call.body),
  ).toEqual([[false, true], ["Swarm Code"], [0.1, 0.8, 0.2, 0.1]]);
  expect(bus.disconnect).toHaveBeenCalledOnce();
  feedback.close();
  expect(bus.disconnect).toHaveBeenCalledOnce();
});
...
it("can activate without animation, and expires a renderer-abandoned capture", async () => {
  ...
  await expect(snapshot!.feedback!.activate("Swarm Code")).rejects.toThrow("No window");
