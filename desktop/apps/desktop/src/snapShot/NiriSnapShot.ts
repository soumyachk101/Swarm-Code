...
async function activateNiriWindow(path: string, title: string, signal: AbortSignal): Promise<void> {
  ...
  try {
    const target = connection.waitFor((value) => {
      ...
      // Never activate another process's lookalike window, or guess between multiple Swarm Code windows.
      const matches = [...windows.values()].filter(
        (window) => window.pid === process.pid && window.title === title,
      );
      if (matches.length > 1)
        throw new Error("More than one Swarm Code window matches the capture destination.");
      return matches[0];
    });
...
export async function captureNiriWindow(path: string): Promise<LinuxWindowSnapshot> {
  const directory = await NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "swarm-code-niri-capture-"));
