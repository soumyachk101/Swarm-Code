...
import type { DesktopCaptureExtensionState } from "@swarmcode/contracts";
...
export async function installGnomeCaptureBundle({ bundle, dataHome }: SetupPaths) {
  ...
  if (existing) {
    ...
    if (installed.version > metadata.version)
      throw new Error("A newer extension is installed. Update Swarm Code instead of replacing it.");
  }
  const staged = await NodeFSP.mkdtemp(NodePath.join(parent, ".swarm-code-capture-install-"));
  ...
  try {
    ...
    if (existing) {
      const backupParent = NodePath.join(dataHome, "swarmcode", "extension-backups");
    ...
    try {
      ...
      if (installed && (!info.state || (info.version && installed.version > info.version.value)))
        return {
          ...
          message:
            "Installed. Save your work, sign out of GNOME and sign back in, then return here to enable the extension. Restarting Swarm Code alone is not enough.",
        };
      ...
      if (!properties.UserExtensionsEnabled.value)
        return {
          ...
          message:
            "GNOME has disabled user extensions. Turn on Extensions in the GNOME Extensions app, then check again. Swarm Code will not enable your other extensions for you.",
        };
      if (info.state?.value === 1)
        return {
          status: "enabled",
          message: "The Swarm Code extension is running. Active-window snapshots are available.",
        };
      ...
      return {
        status: "disabled",
        message:
          "Enable the Swarm Code extension to allow active-window snapshots. You can disable it here at any time.",
      };
    } catch (error) {
      return {
        status: "error",
