...
import type { DesktopCaptureHelperState } from "@swarmcode/contracts";
...
export const KDE_CAPTURE_EXECUTABLE = "swarm-code-kde-snap-shot";
const DESKTOP_FILE = "com.swarmcode.swarmcode.KdeCapture.desktop";
const MARKER = "X-SwarmCode-Capture-Helper=true";
...
export function kdeCapturePaths(paths: KdeCapturePaths) {
  return {
    executable: NodePath.join(paths.dataHome, "swarm-code", "kde-capture", KDE_CAPTURE_EXECUTABLE),
...
export class KdeCaptureSetup {
  ...
  async state(): Promise<DesktopCaptureHelperState> {
    try {
      ...
      if (!bundle)
        return {
          status: "error",
          message: "The capture helper is missing from this build. Update or reinstall Swarm Code.",
        };
  ...
  async perform(action: "install-kde-helper" | "remove-kde-helper") {
    ...
    if (action === "remove-kde-helper") {
      ...
      if (!bundle)
        throw new Error(
          "The capture helper is missing from this build. Update or reinstall Swarm Code.",
        );
...
export async function captureKdeWindow(
  ...
  const directory = await NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "swarm-code-kde-capture-"));
  let retained = false;
      ...
      feedback: {
        ...
        activate: async (title) => {
          ...
          const activation = await NodeFSP.mkdtemp(
            NodePath.join(NodeOS.tmpdir(), "swarm-code-kde-activate-"),
          );
          try {
            await run(
              executable,
