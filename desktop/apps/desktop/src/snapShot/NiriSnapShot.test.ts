...
beforeEach(async () => {
  directory = await NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "swarm-code-niri-test-"));
...
it("selects the native adapter without needing a portal or GNOME extension", async () => {
  expect(await getLinuxCaptureSupport("com.swarmcode.swarmcode")).toEqual({
    linuxBackend: "niri",
    linuxFeedbackAvailable: false,
  });
  const snapshot = await captureLinuxWindow("com.swarmcode.swarmcode");
...
it("does not activate Swarm Code until requested, then matches PID and title", async () => {
  ...
  windows = [
    { ...window, id: 1, pid: 999, title: "Swarm Code" },
    { ...window, id: 2, pid: process.pid, title: "Other Swarm Code" },
    { ...window, id: 3, pid: process.pid, title: "Swarm Code" },
  ];
  await snapshot.feedback!.activate("Swarm Code");
  expect(calls).toContainEqual({ Action: { FocusWindow: { id: 3 } } });
});

it("waits for the restored Swarm Code window to map instead of polling", async () => {
  const snapshot = await captureNiriWindow(socketPath);
  ...
  handler = async (request, socket) => {
    ...
    if (request === "EventStream")
      send(socket, {
        WindowOpenedOrChanged: { window: { ...window, id: 4, pid: process.pid, title: "Swarm Code" } },
      });
  };
  await snapshot.feedback!.activate("Swarm Code");
...
it("rejects ambiguous activation targets", async () => {
  const snapshot = await captureNiriWindow(socketPath);
  windows = [1, 2].map((id) => ({ ...window, id, pid: process.pid, title: "Swarm Code" }));
  await expect(snapshot.feedback!.activate("Swarm Code")).rejects.toThrow("More than one");
});
...
it("cancels pending activation when capture feedback is closed", async () => {
  ...
  const activation = expect(snapshot.feedback!.activate("Swarm Code")).rejects.toThrow("cancelled");
...
it("rejects compositor errors and cleans up its temporary image", async () => {
  ...
  await expect(captureLinuxWindow("com.swarmcode.swarmcode")).rejects.toThrow("window disappeared");
  expect(await NodeFSP.stat(NodePath.dirname(capturePath!)).catch(() => undefined)).toBeUndefined();
});
