...
beforeEach(async () => {
  directory = await NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "swarm-code-hypr-test-"));
  paths = {
...
it.each([true, false])("uses the active config syntax and user bindings (Lua: %s)", async (lua) => {
  ...
  const result = await hyprlandCaptureShortcut("com.swarmcode.swarmcode", directory);
  expect(result.shortcutConfigPath).toBe(
    NodePath.join(config, lua ? "bindings.lua" : "bindings.conf"),
  );
  expect(result.shortcutBinding).toBe(
    lua
      ? 'hl.bind("CTRL + SHIFT + 2", hl.dsp.global("com.swarmcode.swarmcode:capture-window"))'
      : "bind = CTRL SHIFT, 2, global, com.swarmcode.swarmcode:capture-window",
  );
...
it("captures exact-window metadata for accessibility and focuses only through the installed helper", async () => {
  ...
  await result.feedback?.activate("Swarm Code destination");
  expect(execute.mock.calls.find(([, args]) => args[0] === "activate")?.slice(0, 2)).toEqual([
    hyprlandCaptureExecutable(paths),
    ["activate", String(process.pid), "Swarm Code destination"],
  ]);
