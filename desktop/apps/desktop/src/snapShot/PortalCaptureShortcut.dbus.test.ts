...
it.runIf(NodeChildProcess.spawnSync("dbus-daemon", ["--version"]).status === 0)(
  ...
  async () => {
    const dir = await NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "swarm-code-shortcut-dbus-"));
    ...
    try {
      ...
      server.addMethodHandler((message: Message) => {
        if (message.member === "Register") {
          expect(message.body[0]).toBe("com.swarmcode.swarmcode");
