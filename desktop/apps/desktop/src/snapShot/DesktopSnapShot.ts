...
const MAC_SCREEN_CAPTURE_PERMISSION_MESSAGE =
  "Allow Screen Recording in System Settings, then restart Swarm Code.";
const MAC_ACCESSIBILITY_PERMISSION_MESSAGE =
  "Allow Accessibility in System Settings, then restart Swarm Code.";
const MAC_BOTH_PERMISSIONS_MESSAGE =
  "Allow Accessibility and Screen Recording in System Settings, then restart Swarm Code.";
...
export class DesktopSnapShot extends Context.Service<
  DesktopSnapShot,
  {
    ...
    /** Capture the foreground window in place, including Swarm Code itself. */
...
>()("@swarmcode/desktop/snapShot/DesktopSnapShot") {}
...
export const make = Effect.gen(function* () {
    ...
    return yield* Effect.gen(function* () {
      ...
      if (snapshot.linuxActivationFailure) {
        yield* Effect.logWarning(
          "The compositor could not activate Swarm Code after the snapshot",
  ...
  const checkShortcut = Effect.fn("desktop.snapShot.checkShortcut")(function* (
    ...
    if (mode === "portal" && niriSocketPath()) {
      return {
        available: false,
        message: "Configure the capture shortcut in your Niri config, not in Swarm Code.",
      };
    }
    ...
    if (mode === "portal" && niriSocketPath()) {
      const registered = yield* Effect.tryPromise(async () => {
        ...
        return startNiriCaptureShortcut(linuxAppId, onCurrentShortcut, () => {
          void runPromise(
            setShortcutFailure("The Niri capture endpoint disconnected. Restart Swarm Code."),
          ).catch(() => undefined);
        });
      }).pipe(
        Effect.tap((stop) =>
      ...
      yield* Ref.set(stateRef, {
        ...
        shortcutMessage: registered
          ...
          : "Could not start the Niri capture endpoint. Another Swarm Code instance may be using it.",
        message: null,
      });
      return;
    }
      ...
      yield* Effect.scoped(
        Effect.gen(function* () {
          const directory = yield* fileSystem.makeTempDirectoryScoped({
            prefix: "swarm-code-snapshot-test-",
          });
          yield* Effect.tryPromise(async () => {
