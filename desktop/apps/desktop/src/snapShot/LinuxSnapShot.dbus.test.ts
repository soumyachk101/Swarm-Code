...
it.runIf(hasDbus)("captures through real D-Bus marshalling on a private bus", async () => {
  const directory = await NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "swarm-code-private-dbus-"));
  ...
  try {
    ...
    await server.requestName("org.gnome.Shell.Extensions.SwarmCodeSnapShot", NameFlag.DO_NOT_QUEUE);
    ...
    expect(await portal.backend("com.swarmcode.swarmcode")).toBe("screenshot-portal");
    expect(await portal.capturePortal()).toEqual({ png });
    expect(target).toBe(8);
    portalVersion = 2;
    const extension = connect();
    expect(await extension.backend("com.swarmcode.swarmcode")).toBe("gnome-extension");
    expect(await extension.captureExtension("com.swarmcode.swarmcode")).toMatchObject({
      png,
      window: { processId: 42 },
    });
    const owner = await server.call(
      new Message({
        ...
        member: "GetNameOwner",
        signature: "s",
        body: ["com.swarmcode.swarmcode.SnapShot"],
      }),
    );
    ...
    expect(await updated.backend("com.swarmcode.swarmcode")).toBe("gnome-extension");
    const snapshot = await updated.captureExtension("com.swarmcode.swarmcode", {
      flash: true,
      animate: true,
    });
    expect(snapshot.feedback?.animationStarted).toBe(true);
    await snapshot.feedback!.activate("Swarm Code");
    ...
    expect(activateTitle).toBe("Swarm Code");
    expect(animateFrame).toEqual([0.1, 0.8, 0.2, 0.1]);
    vi.stubEnv("XDG_CURRENT_DESKTOP", "KDE");
    const kde = connect();
    expect(await kde.backend("com.swarmcode.swarmcode")).toBe("kde");
    ...
    stopNiriShortcut = await startNiriCaptureShortcut(
      "com.swarmcode.swarmcode.NiriTest",
      triggered,
      failed,
      niriBus,
    );
    const command = [
      ...
      "--session",
      "--dest",
      "com.swarmcode.swarmcode.NiriTest.SnapShot",
      "--object-path",
      "/com/swarmcode/SnapShot",
      "--method",
      "com.swarmcode.SnapShot.Capture",
    ];
    ...
    const invalid = server.call(
      new Message({
        destination: "com.swarmcode.swarmcode.NiriTest.SnapShot",
        path: "/com/swarmcode/SnapShot",
        interface: "com.swarmcode.SnapShot",
        member: "Capture",
        signature: "s",
        body: ["not allowed"],
      }),
