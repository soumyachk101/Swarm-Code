...
export class CaptureShortcutConfig {
  ...
  async apply(id: string, desktop: Target["desktop"]): Promise<DesktopCaptureConfigApplied> {
    ...
    const temporary = NodePath.join(directory, `.swarm-code-capture-${NodeCrypto.randomUUID()}.tmp`);
    let staged = false;
    try {
      ...
      await unchanged();
      const backupPath = `${root.resolvedPath}.swarm-code-capture-backup-${NodeCrypto.randomUUID()}`;
