...
import * as SshAuth from "@swarmcode/ssh/auth";
import { resolveSshTarget } from "@swarmcode/ssh/command";
import { discoverSshHosts } from "@swarmcode/ssh/config";
import {
  SshCommandError,
  ...
  SshReadinessError,
} from "@swarmcode/ssh/errors";
import * as SshTunnel from "@swarmcode/ssh/tunnel";
...
>()("@swarmcode/desktop/ssh/DesktopSshEnvironment") {}
...
export function toSshPasswordPromptError(
  ...
  switch (cause._tag) {
    ...
    case "DesktopSshPromptPresentationError":
      message = "Swarm Code window is not available for SSH authentication.";
      break;
