...
const makeEnvironment = (overrides: Record<string, unknown> = {}) =>
  DesktopEnvironment.DesktopEnvironment.of({
    ...
    isPackaged: true,
    isDevelopment: false,
    displayName: "Swarm Code (Alpha)",
    linuxDesktopEntryName: "com.swarmcode.swarmcode.desktop",
    linuxWmClass: "swarmcode",
    linuxApplicationsDir: "/home/alice/.local/share/applications",
    appImagePath: Option.some("/home/alice/Applications/Swarm-Code.AppImage"),
...
describe("DesktopLinuxUrlHandler", () => {
  it("renders a scheme-handler desktop entry with freedesktop Exec quoting", () => {
    const entry = DesktopLinuxUrlHandler.renderUrlHandlerDesktopEntry({
      displayName: "Swarm Code (Nightly)",
      execTarget: '/home/al ice/Apps/Swarm Code "100%" $HOME\\x.AppImage',
      scheme: "swarmcode",
    });

    assert.include(entry, "[Desktop Entry]");
    assert.include(entry, "Name=Swarm Code (Nightly)");
    ...
    assert.include(
      entry,
      'Exec="/home/al ice/Apps/Swarm Code \\\\"100%%\\\\" \\\\$HOME\\\\\\\\x.AppImage" %U',
    );
    assert.include(entry, "NoDisplay=true");
    assert.notInclude(entry, "StartupWMClass=");
    assert.include(entry, "MimeType=x-scheme-handler/swarmcode;");
  });
  ...
  it("carries structured context on registration errors", () => {
    const writeError = new DesktopLinuxUrlHandler.DesktopLinuxUrlHandlerRegistrationError({
      ...
      scheme: "t3code",
      desktopEntryPath: "/home/alice/.local/share/applications/com.swarmcode.swarmcode.desktop",
    ...
    assert.equal(
      ...
      "/home/alice/.local/share/applications/com.swarmcode.swarmcode.desktop",
    );
  ...
  it.effect("writes the handler entry and claims the scheme default via xdg-mime", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.deepEqual(recorded.commands, [
        {
          command: "xdg-mime",
          args: ["default", "com.swarmcode.swarmcode.desktop", "x-scheme-handler/t3code"],
        },
      ]);
    });
  });
  ...
  it.effect("writes the portal identity without claiming the URL scheme in development", () => {
    ...
    return Effect.gen(function* () {
      ...
      yield* runRegister(unpackaged, {
        environment: {
          isPackaged: false,
          linuxDesktopEntryName: "com.swarmcode.swarmcode.Development.desktop",
        },
      });
      ...
      assert.equal(
        ...
        "/home/alice/.local/share/applications/com.swarmcode.swarmcode.Development.desktop",
      );
    ...
    return Effect.gen(function* () {
      ...
      yield* runRegister(writeFailed, {
        writeError: PlatformError.systemError({
          ...
          pathOrDescriptor: "/home/alice/.local/share/applications/com.swarmcode.swarmcode.desktop",
        }),
      });
