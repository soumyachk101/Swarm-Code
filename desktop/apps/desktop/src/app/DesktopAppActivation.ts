...
import { HostProcessUserId } from "@swarmcode/shared/hostProcess";
  ...
  const server = NodeNet.createServer((socket) => {
    ...
    socket.on("data", (chunk) => {
      ...
      void input.handle(parsed).then(finish, () => {
        finish(
          invalidResponse(parsed.requestId, "Swarm Code could not process the desktop app request."),
        );
      });
    });
...
>()("@swarmcode/desktop/app/DesktopAppActivation") {}
