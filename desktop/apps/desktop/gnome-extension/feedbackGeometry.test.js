...
it("activates only a window of the process authenticated by D-Bus", () => {
  const target = window(42, "Swarm Code");
  expect(findCaptureDestination([window(99, "Swarm Code"), target], 42, "Swarm Code")).toBe(target);
  expect(findCaptureDestination([window(99, "Swarm Code")], 42, "Swarm Code")).toBeUndefined();
...
it("does not guess between ambiguous windows from the same process", () => {
  expect(findCaptureDestination([window(42, "Swarm Code"), window(42, "Swarm Code")], 42, "Swarm Code")).toBeUndefined();
