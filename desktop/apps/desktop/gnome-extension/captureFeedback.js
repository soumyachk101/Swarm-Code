...
export class CaptureFeedback {
  begin(sender, pid, snapshot, options) {
    ...
    try {
      ...
      this._clearActors();
      console.warn(`Swarm Code capture effects unavailable: ${error.message}`);
    }
  ...
  async activate(sender, title) {
    ...
    // A command-palette capture temporarily unmaps Swarm Code. Wait for its new surface, not a sleep.
    const target =
      find() ??
    ...
    if (this._owned(sender) !== session || !target)
      throw new Error("Swarm Code's window is not available for activation.");
