...
it.layer(
  ...
  it.effect("attaches to exited sessions without restarting them", () =>
    Effect.gen(function* () {
      ...
      const unsubscribe = yield* manager.attachStream(
        openInput({
          env: {
            SWARMCODE_WORKTREE_PATH: "/tmp/should-not-restart",
          },
      ...
      const unsubscribe = yield* manager.attachStream(
        {
          ...openInput({
            env: {
              SWARMCODE_WORKTREE_PATH: "/tmp/restart-requested",
            },
  ...
  it.effect("filters app runtime env variables from terminal sessions", () =>
    Effect.gen(function* () {
      ...
      expect(spawnInput.env.SWARMCODE_PORT).toBeUndefined();
  ...
  it.effect("injects runtime env overrides into spawned terminals", () =>
    Effect.gen(function* () {
      ...
      yield* manager.open(
        openInput({
          env: {
            SWARMCODE_PROJECT_ROOT: "/repo",
            SWARMCODE_WORKTREE_PATH: "/repo/worktree-a",
            CUSTOM_FLAG: "1",
            NO_COLOR: "1",
            FORCE_COLOR: "0",
          },
      ...
      assert.equal(spawnInput.env.SWARMCODE_PROJECT_ROOT, "/repo");
      assert.equal(spawnInput.env.SWARMCODE_WORKTREE_PATH, "/repo/worktree-a");
  ...
  it.effect("resolves a provider instance environment before spawning", () =>
    Effect.gen(function* () {
      ...
      const { manager, ptyAdapter } = yield* createManager(5, {
        env: { SWARMCODE_SECRET: "server-only" },
      ...
      expect(ptyAdapter.spawnInputs[0]?.env.SWARMCODE_SECRET).toBeUndefined();
