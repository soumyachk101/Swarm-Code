...
import type { ChatAttachment, ProviderApprovalDecision, RuntimeMode } from "@swarmcode/contracts";
import {
...
import { compareSemverVersions, parseSemver } from "@swarmcode/shared/semver";
...
export const verifyOpenCodeServerVersion = Effect.fn("verifyOpenCodeServerVersion")(function* (
    ...
    Effect.mapError(
      (cause) =>
        new OpenCodeRuntimeError({
          operation: "global.health",
          detail: `OpenCode server returned an invalid health response. Swarm Code requires OpenCode v${MINIMUM_OPENCODE_VERSION} or newer.`,
          cause,
        }),
    ),
  );
  if (parseSemver(health.version) === null) {
    return yield* new OpenCodeRuntimeError({
      operation: "global.health",
      detail: `OpenCode server returned an invalid version. Swarm Code requires OpenCode v${MINIMUM_OPENCODE_VERSION} or newer.`,
    });
  }
