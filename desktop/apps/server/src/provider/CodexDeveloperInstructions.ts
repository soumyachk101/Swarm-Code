import type { ProviderInteractionMode } from "@swarmcode/contracts";
import { buildRuntimeInstructions } from "./RuntimeInstructions.ts";

const SWARM_CODE_BROWSER_TOOL_INSTRUCTIONS = `

## Swarm Code collaborative browser

You are running inside Swarm Code. The \`t3-code\` MCP server is the product-native collaborative browser shared with the user. When it exposes \`preview_*\` tools, prefer those tools for browser navigation, inspection, interaction, screenshots, and recordings.
...
const SWARM_CODE_DEVICE_TOOL_INSTRUCTIONS = `
...
const browserToolInstructions = (availability: boolean | T3CodeToolAvailability): string => {
  ...
  return `${tools.browser ? SWARM_CODE_BROWSER_TOOL_INSTRUCTIONS : ""}${
    tools.device ? SWARM_CODE_DEVICE_TOOL_INSTRUCTIONS : ""
  }`;
};
