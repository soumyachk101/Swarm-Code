...
import { deepMerge } from "@swarmcode/shared/Struct";
import { createModelCapabilities } from "@swarmcode/shared/model";
import { applyServerSettingsPatch } from "@swarmcode/shared/serverSettings";
...
process.env.SWARMCODE_CURSOR_ENABLED = "1";
    ...
    describe("ProviderRegistryLive", () => {
          ...
          Effect.gen(function* () {
            ...
            yield* Effect.gen(function* () {
              ...
              assert.strictEqual(
                ...
                "Cursor is disabled in Swarm Code settings.",
              );
      ...
      it.effect("skips codex probes entirely when the provider is disabled", () =>
        Effect.gen(function* () {
          ...
          assert.strictEqual(status.installed, false);
          assert.strictEqual(status.message, "Codex is disabled in Swarm Code settings.");
        }),
      );
    });
