...
import { expandAssistantCitationsForProvider } from "@swarmcode/shared/assistantCitations";
import { HostProcessPlatform } from "@swarmcode/shared/hostProcess";
import { causeErrorTag } from "@swarmcode/shared/observability";
import { getModelSelectionStringOptionValue } from "@swarmcode/shared/model";
    ...
    function* (threadId, rawInput) {
      ...
      return yield* Effect.gen(function* () {
        ...
        if (!instanceInfo.enabled) {
          return yield* toValidationError(
            "ProviderService.startSession",
            `Provider instance '${resolvedInstanceId}' is disabled in Swarm Code settings.`,
          );
        }
