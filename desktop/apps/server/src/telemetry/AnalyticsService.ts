...
import type { ClientOs } from "@swarmcode/contracts";
...
const TelemetryEnvConfig = Config.all({
  posthogKey: Config.String("SWARMCODE_POSTHOG_KEY").pipe(
    Config.withDefault("phc_XOWci4oZP4VvLiEyrFqkFjP4CZn55mjYYBMREK5Wd6m"),
  ),
  posthogHost: Config.String("SWARMCODE_POSTHOG_HOST").pipe(
    Config.withDefault("https://us.i.posthog.com"),
  ),
  enabled: Config.Boolean("SWARMCODE_TELEMETRY_ENABLED").pipe(Config.withDefault(true)),
  flushBatchSize: Config.Number("SWARMCODE_TELEMETRY_FLUSH_BATCH_SIZE").pipe(Config.withDefault(20)),
  maxBufferedEvents: Config.Number("SWARMCODE_TELEMETRY_MAX_BUFFERED_EVENTS").pipe(
