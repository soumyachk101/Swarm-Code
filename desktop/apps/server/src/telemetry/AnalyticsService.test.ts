...
it.layer(NodeServices.layer)("AnalyticsService test", (it) => {
    ...
    Effect.gen(function* () {
      ...
      const configLayer = ConfigProvider.layer(
        ConfigProvider.fromUnknown({
          SWARMCODE_TELEMETRY_ENABLED: true,
          SWARMCODE_POSTHOG_KEY: "phc_test_key",
          SWARMCODE_POSTHOG_HOST: "http://localhost",
          SWARMCODE_TELEMETRY_FLUSH_BATCH_SIZE: 20,
        }),
      );
  ...
  it.effect("does not send batch requests when telemetry is disabled", () =>
    Effect.gen(function* () {
      ...
      const configLayer = ConfigProvider.layer(
        ConfigProvider.fromUnknown({
          SWARMCODE_TELEMETRY_ENABLED: false,
          SWARMCODE_POSTHOG_KEY: "phc_test_key",
          SWARMCODE_POSTHOG_HOST: "http://localhost",
        }),
      );
