...
export class Launcher {
  ...
  async #recover(): Promise<void> {
    ...
    // handoff release its tunnel. A restart deferred by `swarmcode update` is done
  ...
  async #startChild(version: string, role: ChildRole, update?: ServiceUpdateRecord): Promise<void> {
    ...
    if (!(await runtimeExists(this.#baseDir, version))) {
      throw new Error(`Selected swarmcode@${version} runtime is missing or incomplete.`);
    }
    if (this.#stopping) return;
...
export async function main(): Promise<void> {
  const baseDir = process.env.SWARMCODE_HOME?.trim();
  if (baseDir === undefined || baseDir === "") {
    throw new Error("SWARMCODE_HOME is required by the Swarm Code service launcher.");
  }
