...
import {
  ...
  OtlpProtocol,
  type SignalExport,
} from "@swarmcode/shared/observability";
import { parsePersistedServerObservabilitySettings } from "@swarmcode/shared/serverSettings";
import { DesktopBackendBootstrap, PortSchema } from "@swarmcode/contracts";
...
export const baseDirFlag = Flag.String("base-dir").pipe(
  Flag.withDescription(
    "Explicit Swarm Code data directory; runtime state is stored under userdata (equivalent to SWARMCODE_HOME).",
  ),
  Flag.optional,
);
...
const logWebSocketEventsFlag = Flag.Boolean("log-websocket-events").pipe(
  Flag.withDescription(
    "Emit server-side logs for outbound WebSocket push traffic (equivalent to SWARMCODE_LOG_WS_EVENTS).",
  ),
...
const EnvServerConfig = Config.all({
  logLevel: Config.LogLevel("SWARMCODE_LOG_LEVEL").pipe(Config.withDefault("Info")),
  traceMinLevel: Config.LogLevel("SWARMCODE_TRACE_MIN_LEVEL").pipe(Config.withDefault("Info")),
  traceTimingEnabled: Config.Boolean("SWARMCODE_TRACE_TIMING_ENABLED").pipe(Config.withDefault(true)),
  traceFile: Config.String("SWARMCODE_TRACE_FILE").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  traceMaxBytes: Config.Int("SWARMCODE_TRACE_MAX_BYTES").pipe(Config.withDefault(10 * 1024 * 1024)),
  traceMaxFiles: Config.Int("SWARMCODE_TRACE_MAX_FILES").pipe(Config.withDefault(10)),
  traceBatchWindowMs: Config.Int("SWARMCODE_TRACE_BATCH_WINDOW_MS").pipe(Config.withDefault(1_000)),
  otlpTracesUrl: Config.String("SWARMCODE_OTLP_TRACES_URL").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  otlpMetricsUrl: Config.String("SWARMCODE_OTLP_METRICS_URL").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  otlpLogsUrl: Config.String("SWARMCODE_OTLP_LOGS_URL").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  ...
  otlpServiceName: Config.String("SWARMCODE_OTLP_SERVICE_NAME").pipe(Config.withDefault("t3-server")),
  otlpHeaders: Config.schema(OtlpHeadersFromString, "SWARMCODE_OTLP_HEADERS").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  ...
  mode: Config.schema(ServerConfig.RuntimeMode, "SWARMCODE_MODE").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  port: Config.Port("SWARMCODE_PORT").pipe(Config.option, Config.map(Option.getOrUndefined)),
  host: Config.String("SWARMCODE_HOST").pipe(Config.option, Config.map(Option.getOrUndefined)),
  t3Home: Config.String("SWARMCODE_HOME").pipe(Config.option, Config.map(Option.getOrUndefined)),
  devUrl: Config.URL("VITE_DEV_SERVER_URL").pipe(Config.option, Config.map(Option.getOrUndefined)),
  devAllowedOrigins: Config.String("SWARMCODE_DEV_ALLOWED_ORIGINS").pipe(
    Config.withDefault(""),
    Config.map((value) =>
      value
        .split(",")
  ...
  noBrowser: Config.Boolean("SWARMCODE_NO_BROWSER").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  bootstrapFd: Config.Int("SWARMCODE_BOOTSTRAP_FD").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  autoBootstrapProjectFromCwd: Config.Boolean("SWARMCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  logWebSocketEvents: Config.Boolean("SWARMCODE_LOG_WS_EVENTS").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  tailscaleServeEnabled: Config.Boolean("SWARMCODE_TAILSCALE_SERVE").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  tailscaleServePort: Config.Port("SWARMCODE_TAILSCALE_SERVE_PORT").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
});

const DevAuthTokenConfig = Config.Redacted("SWARMCODE_DEV_AUTH_TOKEN").pipe(
          ...
          new Config.ConfigError(
            new Schema.SchemaError(
              new SchemaIssue.InvalidValue({
                message: "SWARMCODE_DEV_AUTH_TOKEN must contain at least 32 characters.",
              }),
            ),
          ),
        ),
...
export const resolveServerConfig = (
  ...
  Effect.gen(function* () {
    ...
    // Swarm Code's own OTLP variables name no signal, so the one answer they give
