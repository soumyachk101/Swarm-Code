...
import { HostProcessArguments } from "@swarmcode/shared/hostProcess";
 ...
 * Detection is best-effort; callers must fail closed to a plain `swarmcode` command.
 */
...
/**
 * The `swarmcode` package spec to suggest. The literal spec the user typed (e.g.
 * `swarmcode@nightly`) is resolved away before our process starts, so re-derive it
...
function suggestedPackageSpec(version: string): string {
  ...
  return channel === undefined ? "swarmcode" : `swarmcode@${channel}`;
}

/**
 * Render a `swarmcode <subcommand>` suggestion that matches how this process was
 * launched, so copy/pasting it actually works: `npx swarmcode connect` suggests
 * `npx swarmcode serve`, a global install suggests `swarmcode serve`, and a nightly build
...
export function formatCliCommand(input: {
  ...
  if (runner === null) {
    return `swarmcode ${input.subcommand}`;
  }
