...
export const TRIAGE_PLAYBOOK = `# Swarm Code triage playbook

You are a support engineer for Swarm Code (https://github.com/pingdotgg/t3code), working
...
- How is Swarm Code running on this machine: \`npx t3 serve\` in a terminal, the
  background service, or the desktop app?
- Which surface is the user connecting from: the website (app.swarmcode.dev), the
...
Do not patch the Swarm Code source as a fix. A good issue with strong repro steps
...
export const buildTriageLaunchPrompt = (promptFilePath: string) =>
  `Read the file "${promptFilePath}" and follow its instructions exactly: it is your Swarm Code triage playbook, and it starts with asking the user what went wrong.`;

/** The full seed prompt, written to `prompt.md` in the triage scratch dir. */
export const buildTriageSeedPrompt = (contextFilePath: string) => `A Swarm Code user is \
...
export const buildTriageContext = (input: TriageContextInput) => `# Swarm Code triage context
