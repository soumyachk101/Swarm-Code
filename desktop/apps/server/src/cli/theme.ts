...
import { fromJsonStringPretty, fromLenientJson } from "@swarmcode/shared/schemaJson";
import { BUILT_IN_THEME_IDS, UNPUBLISHABLE_THEME_IDS } from "@swarmcode/shared/themePalettes";
...
export class ThemeFileInvalidError extends Schema.TaggedError<ThemeFileInvalidError>()(
  ...
  override get message(): string {
    return `${this.filePath} is not a valid theme file. Use a theme exported from Swarm Code, or a seeded file with name, appearance, canvas, and accent.`;
  }
}
...
const envT3Home = Config.String("SWARMCODE_HOME").pipe(Config.option);

const resolveThemePaths = Effect.fn(function* (explicitBaseDir: Option.Option<string>) {
  // Same precedence as the rest of the CLI: --base-dir, then SWARMCODE_HOME,
  // then the default home. A provisioning script exporting SWARMCODE_HOME must
