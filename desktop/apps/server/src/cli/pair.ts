 ...
 * shared Swarm Code home. `--tailscale` publishes the server over Tailscale Serve
...
import { resolveWorktreeSwarmCodeHome } from "@swarmcode/shared/devHome";
import { DEFAULT_SIGNAL_EXPORT } from "@swarmcode/shared/observability";
import {
  buildTailscaleHttpsBaseUrl,
  DEFAULT_TAILSCALE_SERVE_PORT,
  ensureTailscaleServe,
  readTailscaleStatus,
} from "@swarmcode/tailscale";
...
export class NoRunningServerError extends Schema.TaggedError<NoRunningServerError>()(
  ...
  override get message(): string {
    return [
      "No running Swarm Code server found.",
      ...this.checkedStatePaths.map((statePath) => `  checked ${statePath}`),
      "Start one with `npx t3 serve`, or connect this machine with Swarm Code Connect: `npx t3 connect`.",
    ].join("\n");
  }
}
...
export class ServesOtherEnvironmentError extends Schema.TaggedError<ServesOtherEnvironmentError>()(
  ...
  override get message(): string {
    return `Tailscale Serve on HTTPS port ${String(this.servePort)} already fronts a different Swarm Code server. Pass --tailscale-serve-port to publish this one on another port.`;
  }
}
...
export class ServePortOccupiedError extends Schema.TaggedError<ServePortOccupiedError>()(
  ...
  override get message(): string {
    return `HTTPS port ${String(this.servePort)} on the tailnet already serves something that is not a Swarm Code server. Pass --tailscale-serve-port to publish this one on another port.`;
  }
}
...
/**
 * Three outcomes, because they drive different decisions: a Swarm Code descriptor
 * (pair with it), nothing answering (safe to configure Tailscale Serve), or
 * something answering that is not a Swarm Code server (do NOT overwrite its mapping).
 */
...
const discoverPairTarget = Effect.fn("pair.discoverPairTarget")(function* (
  ...
  if (explicitBaseDir !== undefined && explicitBaseDir.trim().length > 0) {
    ...
    // outranks the shared home, so `swarm-code pair` in a worktree pairs with the dev
    // server under test rather than the daily-driver install.
    const worktreeHome = yield* resolveWorktreeSwarmCodeHome(process.cwd());
    if (worktreeHome !== undefined) {
      bases.push(worktreeHome);
    }
    const envHome = yield* Config.String("SWARMCODE_HOME").pipe(Config.option);
...
const resolveTailscalePairingBase = Effect.fn("pair.resolveTailscalePairingBase")(
  function* (input: { readonly target: DiscoveredPairTarget; readonly servePort: number }) {
    ...
    // environment, is safe to (re)configure. Any other responder — Swarm Code or not
...
export const pairCommand = Command.make("pair", {
  ...
  Command.withDescription(
    "Mint a pairing token for a running Swarm Code server and print it as a QR code.",
  ),
