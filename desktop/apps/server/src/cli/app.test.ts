...
import type { DesktopAppActivationRequest } from "@swarmcode/contracts";
import { resolveDesktopAppControlAddress } from "@swarmcode/shared/desktopAppControl";
import {
  HostProcessPlatform,
  HostProcessUserId,
...
describe("t3 app", () => {
    ...
    withTempDirectory("t3-app-missing-test-", (root) =>
      Effect.gen(function* () {
        ...
        expect(error).toMatchObject({
          ...
          message: expect.stringContaining("Could not reach the Swarm Code desktop app."),
          cause: { code: "ENOENT" },
        });
        expect(yield* pathExists(baseDir)).toBe(false);
      }),
  ...
  it.effect("uses SWARMCODE_HOME or --base-dir and sends the default or explicit path", () =>
    withTempDirectory("t3-app-command-test-", (root) =>
      Effect.gen(function* () {
        ...
        yield* runCli(["app"], { SWARMCODE_HOME: baseDir });
  ...
  it.effect("finds the dev desktop when the default desktop socket is absent", () =>
    withTempDirectory("t3-app-dev-test-", (root) =>
      Effect.gen(function* () {
        ...
        yield* runCli(["app"]);
        yield* runCli(["app"], { SWARMCODE_HOME: "   " });
  ...
  it.effect("never searches a dev state directory for an explicit Swarm Code home", () =>
    withTempDirectory("t3-app-explicit-test-", (root) =>
      Effect.gen(function* () {
        ...
        const envError = yield* runCli(["app"], { SWARMCODE_HOME: baseDir }).pipe(Effect.flip);
