...
it.layer(NodeServices.layer)("cli config resolution", (it) => {
    ...
    Effect.gen(function* () {
      ...
      const configLayer = ConfigProvider.layer(
        ConfigProvider.fromEnv({
          env: {
            SWARMCODE_DEV_AUTH_TOKEN: "  reusable-dev-auth-token-that-is-long-enough  ",
          },
        }),
      );
  ...
  it.effect("does not expose an invalid reusable auth token", () =>
    Effect.gen(function* () {
      ...
      const configLayer = ConfigProvider.layer(
        ConfigProvider.fromEnv({ env: { SWARMCODE_DEV_AUTH_TOKEN: secret } }),
      );
        ...
        Effect.provide(
          Layer.mergeAll(
            ConfigProvider.layer(
              ConfigProvider.fromEnv({
                env: {
                  SWARMCODE_LOG_LEVEL: "Warn",
                  SWARMCODE_MODE: "desktop",
                  SWARMCODE_PORT: "4001",
                  SWARMCODE_HOST: "0.0.0.0",
                  SWARMCODE_HOME: baseDir,
                  VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
                  SWARMCODE_DEV_ALLOWED_ORIGINS:
                    "https://host.example.ts.net, https://phone.example.ts.net ",
                  SWARMCODE_NO_BROWSER: "true",
                  SWARMCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "false",
                  SWARMCODE_LOG_WS_EVENTS: "true",
                },
              }),
            ),
            NetService.layer,
            ...
            ConfigProvider.layer(
              ConfigProvider.fromEnv({
                env: {
                  ...
                  SWARMCODE_PORT: "4001",
                  SWARMCODE_HOST: "0.0.0.0",
                  SWARMCODE_HOME: join(NodeOS.tmpdir(), "ignored-base"),
                  VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
                  SWARMCODE_NO_BROWSER: "false",
                  SWARMCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "false",
                  SWARMCODE_LOG_WS_EVENTS: "false",
                },
              }),
            ),
            NetService.layer,
        ...
        Effect.provide(
          Layer.mergeAll(
            ConfigProvider.layer(
              ConfigProvider.fromEnv({
                env: {
                  SWARMCODE_BOOTSTRAP_FD: String(fd),
                  SWARMCODE_NO_BROWSER: "true",
                  SWARMCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "true",
                  SWARMCODE_LOG_WS_EVENTS: "true",
                },
              }),
            ),
        ...
        Effect.provide(
          Layer.mergeAll(
            ConfigProvider.layer(
              ConfigProvider.fromEnv({
                env: {
                  SWARMCODE_OTLP_HEADERS: "authorization=Basic%20abc%3D%3D,x-tenant=t3",
                },
              }),
            ),
            NetService.layer,
        ...
        Effect.provide(
          Layer.mergeAll(
            ConfigProvider.layer(
              ConfigProvider.fromEnv({
                env: {
                  SWARMCODE_OTLP_HEADERS: "authorization=Bearer abc==, x-tenant=t3",
                  SWARMCODE_OTLP_TRACES_URL: "http://collector.internal:4318",
                },
              }),
            ),
            NetService.layer,
    ...
    Effect.gen(function* () {
      ...
      const resolved = yield* resolveServerConfig(
        ...
        Effect.provide(
          Layer.mergeAll(
            ConfigProvider.layer(
              ConfigProvider.fromEnv({ env: { SWARMCODE_OTLP_PROTOCOL: "http/protobuf" } }),
            ),
            NetService.layer,
          ),
        ),
      ...
      const resolved = yield* resolveServerConfig(
        ...
        Effect.provide(
          Layer.mergeAll(
            ConfigProvider.layer(
              ConfigProvider.fromEnv({
                env: { SWARMCODE_OTLP_LOGS_URL: "http://collector.internal:4318/v1/logs" },
              }),
            ),
            NetService.layer,
          ),
