...
const TestLayer = Layer.empty.pipe(
  Layer.provideMerge(SwarmCodeProjectFileLoader.layer),
...
const writeProjectFile = Effect.fn("writeProjectFile")(function* (cwd: string, contents: string) {
  ...
  yield* fileSystem.writeFileString(path.join(cwd, "swarmCode.json"), contents).pipe(Effect.orDie);
});

it.layer(TestLayer)("SwarmCodeProjectFileLoader", (it) => {
  describe("load", () => {
    it.effect("loads and decodes a valid swarmCode.json", () =>
      Effect.gen(function* () {
        const loader = yield* SwarmCodeProjectFileLoader.SwarmCodeProjectFileLoader;
    ...
    it.effect("returns none when swarmCode.json is missing", () =>
      Effect.gen(function* () {
        const loader = yield* SwarmCodeProjectFileLoader.SwarmCodeProjectFileLoader;
        const cwd = yield* makeTempDir;
