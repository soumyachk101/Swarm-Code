...
const macPlan = {
  ...
  unitPath: "/Users/theo/Library/LaunchAgents/com.swarmcode.t3code.service.plist",
};
...
it.layer(NodeServices.layer)("boot service install", (it) => {
      ...
      Effect.gen(function* () {
        ...
        expect(
          ...
          platform === "linux"
            ...
            : [
                "launchctl bootout --wait gui/501/com.swarmcode.t3code.service",
    ...
    Effect.gen(function* () {
      ...
      expect(
        plan.unitPath.endsWith(
          path.join("Library", "LaunchAgents", "com.swarmcode.t3code.service.plist"),
        ),
      ).toBe(true);
      expect(yield* fs.readFileString(plan.unitPath)).toContain(
      ...
      expect(timeouts.get("launchctl bootout --wait gui/501/com.swarmcode.t3code.service")).toEqual(
        Duration.seconds(120),
      );
    }),
  );
  ...
  it.effect("restarts the launch agent when repair fails", () =>
    Effect.gen(function* () {
      ...
      expect(commands.filter((command) => command.startsWith("launchctl "))).toEqual([
        "launchctl bootout --wait gui/501/com.swarmcode.t3code.service",
        "launchctl enable gui/501/com.swarmcode.t3code.service",
  ...
  it.effect("ignores a bootout for an agent that is not loaded", () =>
    Effect.gen(function* () {
      ...
      yield* service.install();
      control.failCommand = "launchctl bootout --wait gui/501/com.swarmcode.t3code.service";
