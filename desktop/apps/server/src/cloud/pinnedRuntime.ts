...
/**
 * A pinned runtime is an exact Swarm Code release archive unpacked into
 ...
 * Node or npm on the machine; the only npm involvement in Swarm Code is the `swarmcode`
 * package for people who prefer `npx swarmcode` or `npm install -g swarmcode`, and even a
 * CLI installed that way pins an archive when it sets up the service.
 */
const PINNED_RUNTIME_DIR = "runtime";
const PINNED_RUNTIME_INSTALL_TIMEOUT = Duration.minutes(10);
const PINNED_RUNTIME_ARCHIVE_FILE = "swarmcode-runtime-archive";
...
export function pinnedRuntimePaths(
  ...
  return {
    versionDir,
    entryPath: path.join(versionDir, platform === "win32" ? "swarmcode.exe" : "swarmcode"),
...
/**
 * Installs the Swarm Code release archive for `version` into the pinned runtime
...
const installFromArchive = Effect.fn("cloud.pinned_runtime.install_archive")(function* (
  ...
  if (platformKey === undefined) {
    return yield* new PinnedRuntimeInstallError({
      step: `selecting a Swarm Code release archive for ${input.platform}-${input.arch}`,
    });
  }
  ...
  const checksums = parseChecksums(
    new TextDecoder().decode(
      yield* fetchReleaseAsset(
        ...
        "downloading the swarmcode release checksums",
      ),
    ),
  );
  ...
  if (expected === undefined) {
    return yield* new PinnedRuntimeInstallError({
      step: `finding ${fileName} in the swarmcode release checksums`,
    });
  }
  const archive = yield* fetchReleaseAsset(
    httpClient,
    `${baseUrl}/${fileName}`,
    "downloading the Swarm Code release archive",
    input.onProgress,
  );
  ...
  const digest = yield* Effect.tryPromise({
    try: () => crypto.subtle.digest("SHA-256", archive),
      ...
      new PinnedRuntimeInstallError({ step: "verifying the Swarm Code release archive", cause }),
  });
  if (Encoding.encodeHex(new Uint8Array(digest)) !== expected) {
    return yield* new PinnedRuntimeInstallError({
      step: "verifying the Swarm Code release archive checksum",
    });
  }
  ...
  yield* fs
    ...
    .pipe(
      Effect.mapError(
        (cause) => new PinnedRuntimeInstallError({ step: "writing the Swarm Code release archive", cause }),
      ),
    );
  input.onProgress?.({ stage: "extract" });
  const extractStep = "extracting the Swarm Code release archive";
  // The archive wraps everything in one directory named after its stem;
  // strip it so the executable lands at <versionDir>/swarmcode.
  yield* input.runner
    .run({
