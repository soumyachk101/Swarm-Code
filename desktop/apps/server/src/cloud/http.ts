...
import {
  ...
  signRelayJwt,
  verifyRelayJwt,
} from "@swarmcode/shared/relayJwt";
import { isSecureRelayUrl } from "@swarmcode/shared/relayUrl";
  ...
  Effect.mapError(
    () =>
      new EnvironmentHttpInternalServerError({
        message: "SWARMCODE_RELAY_URL must be configured as a secure absolute HTTPS origin.",
      }),
  ),
);
...
export const releaseManagedTunnelOnShutdown = Effect.fn(
  ...
  // persisted URL: SWARMCODE_RELAY_URL may have changed since the link was made.
