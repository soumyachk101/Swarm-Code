...
import { CLI_RELEASE_BASE_URL_ENV } from "@swarmcode/shared/cliRelease";
...
// (com.swarmcode.swarmcode), so launchd and TCC records never collide.
const BOOT_SERVICE_LAUNCHD_LABEL = "com.swarmcode.swarmcode.service";
...
/**
 * Reads `SWARMCODE_HOME` back out of a rendered unit or plist. Only values this
...
export function bootServiceBaseDirOf(contents: string): string | undefined {
  const systemd = /^Environment=SWARMCODE_HOME=(.*)$/m.exec(contents)?.[1];
  ...
  const plist = /<key>SWARMCODE_HOME<\/key>\s*<string>([^<]*)<\/string>/.exec(contents)?.[1];
  if (plist !== undefined) {
...
export function renderBootServiceUnit(plan: BootServicePlan): string {
  ...
  return [
    ...
    "Type=simple",
    "WorkingDirectory=%h",
    `Environment=SWARMCODE_HOME=${quoteSystemdValue(plan.baseDir)}`,
...
export class BootServiceInstallError extends Schema.TaggedError<BootServiceInstallError>()(
  ...
  override get message(): string {
    return "Could not set up the Swarm Code background service.";
  }
}
...
export function formatBootServiceProblem(problem: BootServiceProblem): string {
  switch (problem) {
    ...
    case "linger-disabled":
      return 'Lingering is disabled. Swarm Code will stop when your last login session ends and will not start at boot. Run `sudo loginctl enable-linger "$(id -un)"` on this machine, then retry the service command as your normal user.';
