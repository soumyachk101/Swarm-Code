...
import { runSqliteState } from "./swarm-code-sqlite-state.ts";
...
it.layer(NodeServices.layer)("swarm-code-sqlite-state", (it) => {
  it.effect("reports each invalid SQL source with a specific error", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "swarm-code-sqlite-state-input-" });
  ...
  it.effect("queries an isolated database through Effect SQL", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "swarm-code-sqlite-state-query-" });
  ...
  it.effect.skipIf(!symlinksSupported)(
    ...
    () =>
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
        const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "swarm-code-sqlite-state-exec-" });
        yield* createFixtureDatabase(baseDir);
        ...
        const aliasParent = yield* fs.makeTempDirectoryScoped({
          prefix: "swarm-code-sqlite-state-alias-",
        });
