...
import * as NodeSqliteClient from "@swarmcode/shared/nodeSqliteClient";
...
export class SqliteStateDatabaseMissingError extends Schema.TaggedError<SqliteStateDatabaseMissingError>()(
  ...
  override get message(): string {
    return `Database does not exist at '${this.databasePath}'. Start Swarm Code once to run migrations.`;
  }
}
...
export class SqliteStateSharedHomeMutationError extends Schema.TaggedError<SqliteStateSharedHomeMutationError>()(
  ...
  override get message(): string {
    return "Refusing to mutate the shared ~/.swarmcode database. Use an isolated --base-dir.";
  }
}
...
export const runSqliteState = Effect.fn("runSqliteState")(function* (
  ...
  const sharedHome = path.resolve(options.sharedHome ?? path.join(NodeOS.homedir(), ".swarmcode"));
...
const swarmCodeSqliteStateCommand = Command.make(
  "swarm-code-sqlite-state",
  {
    ...
    baseDir: Flag.String("base-dir").pipe(
      Flag.withDescription("Explicit Swarm Code base directory containing userdata/state.sqlite."),
    ),
  ...
  Command.withDescription(
    "Inspect or seed an isolated Swarm Code SQLite database with automatic backups for writes.",
  ),
);

if (import.meta.main) {
  Command.run(swarmCodeSqliteStateCommand, { version: "0.0.0" }).pipe(
