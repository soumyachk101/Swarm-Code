...
} from "@swarmcode/client-runtime/platform";
import {
...
import { bootstrapRemoteBearerSession } from "@swarmcode/client-runtime/authorization";
import { fetchRemoteEnvironmentDescriptor } from "@swarmcode/client-runtime/environment";
import { managedRelayAccountChanges, managedRelaySessionAtom } from "@swarmcode/client-runtime/relay";
import { EnvironmentRpcRequestObserver } from "@swarmcode/client-runtime/rpc";
import {
    ...
    const cloudSession = CloudSession.of({
      ...
      clerkToken: Effect.gen(function* () {
        ...
        if (session === null) {
          return yield* new ConnectionBlockedError({
            reason: "authentication",
            detail: "Sign in to Swarm Connect to connect this environment.",
          });
        }
        ...
        if (token === null) {
          return yield* new ConnectionBlockedError({
            reason: "authentication",
            detail: "The Swarm Connect session is unavailable.",
          });
        }
        return token;
      }),
