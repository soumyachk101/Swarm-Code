...
describe("Pierre file icons", () => {
  ...
  it("extends Pierre with Swarm-Code-specific exact filename icons", () => {
    assert.equal(resolvePierreIconForEntry("AGENTS.md", "file")?.name, "swarm-code-file-icon-agents");
    assert.equal(resolvePierreIconForEntry("pnpm-lock.yaml", "file")?.name, "swarm-code-file-icon-pnpm");
    assert.equal(
      resolvePierreIconForEntry("pnpm-workspace.yaml", "file")?.name,
      "swarm-code-file-icon-pnpm",
    );
  });

  it("ships every custom icon referenced by the extended resolver", () => {
    const customIconNames = new Set(
      Object.values(SWARM_CODE_PIERRE_ICONS.byFileName).filter((name) => name.startsWith("t3-")),
    );
    for (const iconName of customIconNames) {
      assert.include(SWARM_CODE_PIERRE_ICONS.spriteSheet, `id="${iconName}"`);
    }
  });
