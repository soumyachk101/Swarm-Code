...
} from "@swarmcode/client-runtime/connection";
import {
...
} from "@swarmcode/contracts/relay";
...
/**
 * The user's Swarm Connect environments from relay discovery, each with a
  ...
  const connectEnvironment = async (environment: RelayClientEnvironmentRecord) => {
    ...
    if (result._tag === "Success") {
      toastManager.add({
        ...
        description: `Connecting to ${environment.label} through Swarm Connect.`,
      });
      return true;
    }
    ...
    const message =
      cause instanceof Error ? cause.message : "Could not connect the Swarm Connect environment.";
  ...
  return visibleEnvironments.map(({ environment, availability, error, status }) => {
    ...
    const statusText =
      unsupported && !savedEnvironment
        ? "Swarm Connect · Not added · Client not supported"
        : savedConnection
          ? savedConnection.statusText
          : availability === "online"
            ? "Swarm Connect · Not added · Relay online"
            : availability === "offline"
              ? "Swarm Connect · Not added · Relay offline"
              : availability === "checking"
                ? "Swarm Connect · Not added · Checking relay status…"
                : (Option.getOrNull(error)?.message ??
                  "Swarm Connect · Not added · Relay status unavailable");
    if (selection) {
      return (
        <label
