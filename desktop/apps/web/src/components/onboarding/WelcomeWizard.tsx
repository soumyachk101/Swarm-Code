...
import { scopeProjectRef, scopeThreadRef } from "@swarmcode/client-runtime/environment";
import {
  isAtomCommandInterrupted,
  squashAtomCommandFailure,
} from "@swarmcode/client-runtime/state/runtime";
import { CommandId, ProviderDriverKind, ThreadId } from "@swarmcode/contracts";
...
import { useSwarmConnectAuthPrompt } from "../clerk/useSwarmConnectAuthPrompt";
...
export function WelcomeWizard({
  ...
  return (
    <Dialog open disablePointerDismissal onOpenChange={(_, event) => event.cancel()}>
      ...
      >
        <WizardHeader
          title="Set up Swarm Code"
          identity={
            <div className="flex items-center gap-2" role="img" aria-label="Swarm Code">
              <SwarmCodeWordmark className="h-6 w-6 shrink-0" aria-hidden />
              <span className="text-[1.4rem] font-semibold tracking-tight text-foreground">Swarm</span>
...
function ConnectAccountOption({
  ...
  const { openAuthPrompt } = useSwarmConnectAuthPrompt();
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="flex-1">Swarm Connect</span>
      ...
      <CollapsiblePanel keepMounted>
        <div className="px-3 pb-3">
          ...
          <p className="mt-3 text-xs text-muted-foreground">
            Keep Swarm Code running. Select the computers you want to set up above.
          </p>
        </div>
      </CollapsiblePanel>
    </Collapsible>
        ...
        <Collapsible>
          ...
          <CollapsiblePanel className="pt-3">
            ...
            <p className="mt-2 text-xs leading-relaxed text-muted-foreground">
              Start Swarm Code first, or run <code className="font-mono">npx t3 serve</code>. Add{" "}
