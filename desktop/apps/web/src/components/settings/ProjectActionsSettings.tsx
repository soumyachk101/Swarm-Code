import { EnvironmentId, type SwarmCodeProjectFileScript } from "@swarmcode/contracts";
import {
  isAtomCommandInterrupted,
  squashAtomCommandFailure,
} from "@swarmcode/client-runtime/state/runtime";
import { DEFAULT_RESOLVED_KEYBINDINGS } from "@swarmcode/shared/keybindings";
import { ChevronDownIcon, PlusIcon } from "lucide-react";
import { useCallback, useMemo, useState } from "react";
import { useSwarmCodeProjectFileState } from "../../hooks/useSwarmCodeProjectFileScripts";
...
export function ProjectActionsSettings() {
  ...
  // A project's swarmCode.json can declare actions to import. Read it from the
  ...
  const importableScripts = useMemo(
    ...
    [scripts, swarmFile.scripts],
  );
  const importFileScript = useCallback(
    async (fileScript: SwarmCodeProjectFileScript) => {
  ...
  return (
    <SettingsSection id="project-actions" title="Actions">
            ...
            {importableScripts.length > 0 ? (
              <Menu>
                ...
                <MenuPopup align="end" className="w-72">
                  <MenuGroup>
                    <MenuGroupLabel>Import from swarmCode.json</MenuGroupLabel>
      ...
      {swarmFile.status === "invalid" ? (
        <SettingsRow
          title="swarmCode.json is invalid"
          description="A swarmCode.json exists in this checkout but fails to parse, so every action and icon it declares is ignored. Check the JSON syntax and icon values."
