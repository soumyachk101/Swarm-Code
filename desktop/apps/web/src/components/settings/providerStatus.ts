import type { ServerProvider, ServerProviderVersionAdvisory } from "@swarmcode/contracts";
...
export function getProviderSummary(provider: ServerProvider | undefined) {
  ...
  if (!provider.enabled || provider.status === "disabled") {
    return {
      headline: "Disabled",
      detail:
        provider.message ?? "This provider is installed but disabled for new sessions in Swarm Code.",
    };
  }
