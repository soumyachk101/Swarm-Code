...
import { createModelSelection } from "@swarmcode/shared/model";
...
export function ProjectDefaultsSettings({ category }: { category: ProjectSettingsCategory }) {
  ...
  // A checkout's swarmCode.json wins over the environment default when the project
  ...
  const repositoryEnvMode = swarmFile.file?.defaultThreadEnvMode ?? null;
  const inheritedEnvModeLabel =
    workspaceSource === "project"
      ? null
      : repositoryEnvMode
        ? `${resolveEnvModeLabel(repositoryEnvMode)} (swarmCode.json)`
        : null;
      ...
      {category === "general" ? (
        <>
          ...
          <SettingsRow
            ...
            description={
              isProjectScope
                ? "Where new threads in this project start. A swarmCode.json preference applies when the project has no override."
                : "Where new threads start, unless overridden by the project or swarmCode.json."
            }
            status={
