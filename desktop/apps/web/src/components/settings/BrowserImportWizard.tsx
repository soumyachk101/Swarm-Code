...
import type { BrowserImportSource } from "@swarmcode/contracts";
import { BROWSER_IMPORT_FAILURE_COPY } from "@swarmcode/contracts";
...
function FullDiskAccessStep({
  ...
  return (
    <>
      <DialogHeader>
        <DialogTitle>Let Swarm Code read {source.name}&rsquo;s cookies</DialogTitle>
        <DialogDescription>
          To import cookies from {source.name}, Swarm Code needs Full Disk Access. Turn it on in System
      ...
      <DialogPanel>
        ...
        {!permission.isReady(["fullDiskAccess"]) ? (
          <p className="mt-3 text-xs text-muted-foreground">
            {stillRequired
              ? "Access is still required. Quit and reopen Swarm Code if you just allowed it, then retry the import."
              : "If access doesn't update after you allow it, quit and reopen Swarm Code, then retry the import."}
          </p>
        ) : null}
      </DialogPanel>
      <DialogFooter>
