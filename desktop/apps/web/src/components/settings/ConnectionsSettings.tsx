...
import { connectionStatusText } from "@swarmcode/client-runtime/connection";
import {
  isAtomCommandInterrupted,
  squashAtomCommandFailure,
} from "@swarmcode/client-runtime/state/runtime";
...
function SavedBackendListRow({
  ...
  // A saved Swarm Connect machine this device has never reached (unsupported,
...
function CloudLinkSwitch({
  ...
  disabledReason,
  onCheckedChange,
  ariaLabel = "Enable Swarm Connect",
}: {
...
function ConfiguredCloudLinkRow({ canManageRelay }: { readonly canManageRelay: boolean }) {
  ...
  const disabledReason = !isSignedIn
    ? "Sign in to Swarm Connect to manage this environment."
    : !canManageRelay
      ? "Your session does not have permission to manage Swarm Connect access."
      : null;
  ...
  const updateManagedTunnel = async (enabled: boolean) => {
    ...
    if (ok) {
      ...
      toastManager.add({
        type: "success",
        title: enabled
          ? "Swarm Connect linked"
          : publishAgentActivity
            ? "Swarm Connect tunnel disabled"
            : "Swarm Connect unlinked",
        description: enabled
          ? "This environment is available through Swarm Connect."
          : publishAgentActivity
            ? "The managed tunnel was removed. Agent activity publishing stays on."
            : "This environment is no longer available through Swarm Connect.",
      });
    }
    setIsUpdating(false);
  };
    ...
    <>
      {window.desktopBridge ? (
        <SettingsRow
          ...
          description={
            managedTunnelActive
              ? "This environment is available to your other devices through Swarm Connect."
              : "Make this environment available to your other devices through Swarm Connect."
          }
      ...
      <SettingsRow
        ...
        description="Send activity to mobile notifications and Live Activities without Swarm Connect."
        control={
          <CloudLinkSwitch
  ...
  return (
    <Empty className="min-h-52">
      ...
      <EmptyHeader>
        ...
        <EmptyDescription>
          {cloudEnabled
            ? "Click “Add environment” to pair another environment, or connect one from Swarm Connect."
    ...
    return (
      <>
        ...
        {desktopWslState.enabled ? (
          <SettingsRow
            title="WSL only"
            description="Run only the WSL backend. Swarm Code restarts when this changes."
          ...
          >
            <AlertDialogPopup>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  {pendingDesktopServerExposureMode === "network-accessible"
                    ? "Let your other devices connect to Swarm Code over the network. Pair devices to give them access. Swarm Code will restart."
                    : "Devices connected over your local network will disconnect. Existing tunnels, such as Swarm Connect or Tailscale HTTPS, keep working. Swarm Code will restart."}
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogClose
            ...
            <AlertDialogPopup>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  {pendingWslChange?.kind === "disable"
                    ? pendingWslChange.wasWslOnly
                      ? "Swarm Code will restart on the Windows backend. Threads and projects opened against WSL stay safe inside the distro and become available again when you re-enable WSL."
                      : "The WSL backend will stop. Threads and projects opened against WSL stay safe inside the distro, but they'll be unavailable in Swarm Code until you re-enable WSL."
                    : pendingWslChange?.kind === "distro"
                      ? "Swarm Code will restart the WSL backend on the new distro. Sessions still running on the current distro will be interrupted."
                      : pendingWslChange?.kind === "enable"
                        ? "Run the WSL backend alongside the Windows one, or stop the Windows backend and use only WSL? You can change this later from Settings."
                        : pendingWslChange?.nextValue
                          ? "Swarm Code will restart and start only the WSL backend. Your Windows-side projects won't be accessible until you turn this off again."
                          : "Swarm Code will restart and bring the Windows backend back up alongside WSL."}
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogClose
          ...
          >
            <AlertDialogPopup>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  Swarm Code will restart the local backend without Tailscale Serve.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogClose
          ...
          >
            <DialogPopup className="max-w-md">
              <DialogHeader>
                ...
                <DialogDescription>
                  Swarm Code will restart the local backend with Tailscale Serve enabled and ask
