...
import { EnvironmentId, ProjectId, ProviderInstanceId, ThreadId } from "@swarmcode/contracts";
...
describe("buildThreadActionItems", () => {
  ...
  it("keeps message excerpts searchable without replacing thread metadata", () => {
    const [item] = buildThreadActionItems({
      ...
      projectTitleById: new Map([[PROJECT_ID, "Swarm Code"]]),
      sortOrder: "updated_at",
      icon: null,
      getContentMatch: () => ({
        source: "assistant",
    ...
    expect(item?.description).toBe("Swarm Code · #feat/search");
  });
  ...
  it("prefers renderDescription when provided", () => {
    ...
    expect(item?.description).toBe("Swarm Code:feat/search:wt");
  });
