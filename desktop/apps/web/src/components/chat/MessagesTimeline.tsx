...
import {
  ...
  hasQuestionAnswer,
} from "@swarmcode/client-runtime/work-log/user-input";
import {
...
import { parseScopedThreadKey } from "@swarmcode/client-runtime/environment";
import { replaceComposerContextReferences } from "@swarmcode/shared/composerContextReferences";
...
} from "@swarmcode/client-runtime/work-log/presentation";
import { resolveWorkGroupScrollAnchor } from "@swarmcode/client-runtime/work-log/scroll-anchor";
import type {
  AgentPanelModel,
  RuntimeSubagent,
} from "@swarmcode/client-runtime/state/subagentRuntime";
import { formatAttachmentSize } from "@swarmcode/client-runtime/state/attachments";
import {
...
import { resolveChatListAnchoredEndSpace } from "@swarmcode/shared/chatList";
import { toolActivityFaviconUrl } from "@swarmcode/shared/favicon";
import { formatDuration } from "@swarmcode/shared/orchestrationTiming";
import { getProjectFaviconCacheKey } from "@swarmcode/shared/projectFavicon";
...
import { SwarmCodeWordmark } from "../SwarmCodeWordmark";
import {
  BotIcon,
  BrainIcon,
  CheckIcon,
...
} from "@swarmcode/shared/composerContextReferences";
import {
  COMPOSER_CONTEXT_CLIPBOARD_MIME,
  encodeComposerContextClipboardHtml,
  encodeComposerContextFragment,
} from "@swarmcode/shared/composerContextClipboard";
...
import { type TimestampFormat } from "@swarmcode/contracts/settings";
  ...
  return (
    <>
      <div className="relative min-w-0 px-1 py-0.5">
        <MessageAuthorHeading>Swarm Code</MessageAuthorHeading>
        <AssistantCitationSource
          messageId={row.message.id}
...
function WorkEntryIcon({ name, className }: { name: WorkEntryIconName; className: string }) {
  switch (name) {
    ...
    case "t3-code":
      return <SwarmCodeWordmark className={className} aria-hidden />;
    case "check":
...
function QuestionAnswerHistory({
  ...
  answer: import("@swarmcode/contracts").UserInputAttachmentAnswerPayload;
}) {
  const { activeThreadEnvironmentId } = use(TimelineRowCtx);
