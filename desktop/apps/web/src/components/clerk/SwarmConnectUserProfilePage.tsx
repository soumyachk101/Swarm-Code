import { findErrorTraceId } from "@swarmcode/client-runtime/errors";
import {
  isAtomCommandInterrupted,
  squashAtomCommandFailure,
} from "@swarmcode/client-runtime/state/runtime";
import type { EnvironmentId } from "@swarmcode/contracts";
import type { RelayClientEnvironmentRecord } from "@swarmcode/contracts/relay";
...
export function SwarmConnectEnvironmentRow(props: {
          ...
          <div className="pt-3">
            ...
            >
              ...
              <p className="mt-4 max-w-xl text-[0.8125rem] leading-[1.125rem] text-muted-foreground">
                Swarm Connect access will be revoked, any managed tunnel will be removed, and a host
...
export function SwarmConnectUserProfilePage() {
  ...
  const handleDeregister = async (environment: RelayClientEnvironmentRecord) => {
    ...
    if (result._tag === "Success") {
      ...
      toastManager.add({
        ...
        description: "Swarm Connect access was revoked and a host space is now available.",
      });
      return;
    }
    ...
    console.error("[swarm-connect] Could not deregister environment", {
      environmentId: environment.environmentId,
      message,
      traceId,
      cause,
        ...
        {environmentsState.error ? (
          <div className="mb-4 border-t border-destructive/35 py-3 text-[0.8125rem]" role="alert">
            <p className="font-medium text-destructive-foreground">
              Could not load Swarm Connect environments
            </p>
          ...
          <Empty className="min-h-64 gap-4 border-t px-6 py-10 md:p-10">
            ...
            <EmptyHeader>
              <EmptyTitle className="text-[1.0625rem] leading-6">
                No Swarm Connect environments
              </EmptyTitle>
              <EmptyDescription className="text-[0.8125rem] leading-[1.125rem]">
                Link an environment from its local Settings to make it available through Swarm Connect.
              </EmptyDescription>
            </EmptyHeader>
          </Empty>
        )}
