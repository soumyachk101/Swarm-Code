...
import { BUILT_IN_THEMES } from "@swarmcode/shared/themePalettes";
...
describe("theme files", () => {
  ...
  it("canonicalizes the explicitly exported theme", () => {
    const serialized = serializeThemeFile({
      ...SWARM_CODE_CHAT_THEME,
      colors: { ...SWARM_CODE_CHAT_THEME.colors, accent: "hsl(263 70% 58%)" },
    });
    expect(JSON.parse(serialized)).toMatchObject({
      version: THEME_FILE_VERSION,
      id: SWARM_CODE_CHAT_THEME.id,
      name: SWARM_CODE_CHAT_THEME.label,
      appearance: "light",
  ...
  it("suppresses sidebar artwork during a live custom-theme preview", () => {
    ...
    applyThemeColorPreview(SWARM_CODE_CHAT_THEME.colors, "light");
  ...
  it("keeps optional light and dark palettes under one theme id", () => {
    ...
    expect(getThemeModes(SWARM_CODE_CHAT_THEME)).toEqual(["light", "dark"]);
    expect(resolveThemeAppearance(SWARM_CODE_CHAT_THEME.id, true, true)).toBe("dark");
    expect(resolveDesktopTheme(SWARM_CODE_CHAT_THEME.id, true)).toBe("system");
    expect(resolveThemeAppearance(SWARM_CODE_CHAT_THEME.id, false, false, "dark")).toBe("dark");
    expect(resolveDesktopTheme(SWARM_CODE_CHAT_THEME.id, false, "dark")).toBe("dark");
  ...
  it("keeps the T3 Chat palette faithful and readable", () => {
    expectThemeColors(SWARM_CODE_CHAT_THEME.colors, {
      canvas: "#fdf7fd",
      chrome: "#fdf7fd",
    ...
    expectThemeColors(SWARM_CODE_CHAT_THEME.variants!.dark!, {
      canvas: "#1f1a24",
      chrome: "#1f1a24",
      surface: "#29232d",
    ...
    for (const mode of ["light", "dark"] as const) {
      const colors = getThemeColorsForMode(SWARM_CODE_CHAT_THEME, mode)!;
      expect(contrastRatio(colors.text, colors.canvas)).toBeGreaterThanOrEqual(7);
      expect(contrastRatio(colors.textMuted, colors.canvas)).toBeGreaterThanOrEqual(4.5);
      expect(contrastRatio(colors.messageForeground, colors.messageSurface)).toBeGreaterThanOrEqual(
        4.5,
  ...
  it("includes the dual-mode maintainer themes", () => {
    for (const theme of [SWARM_CODE_CHAT_THEME, GROVE_THEME, OCEAN_THEME, EMBER_THEME, IRIS_THEME]) {
      ...
      for (const mode of ["light", "dark"] as const) {
        ...
        expect(contrastRatio(colors!.textMuted, colors!.canvas)).toBeGreaterThanOrEqual(4.5);
        if (theme !== SWARM_CODE_CHAT_THEME) {
          expect(contrastRatio(colors!.textMuted, colors!.canvas)).toBeLessThan(5.5);
...
describe("stored theme preferences", () => {
  ...
  it("resolves the legacy t3-chat-dark preference to dark T3 Chat", () => {
    expect(getThemeDefinition("swarm-code-chat-dark")).toBe(SWARM_CODE_CHAT_THEME);
    expect(getThemePreferenceMode("swarm-code-chat-dark")).toBe("dark");
    expect(resolveThemeAppearance("swarm-code-chat-dark", true, false)).toBe("dark");
    expect(resolveDesktopTheme("swarm-code-chat-dark", false)).toBe("dark");
    expect(isKnownThemePreference("swarm-code-chat-dark")).toBe(true);
  });
  ...
  it("resolves legacy t3-prefixed ids onto the renamed themes", () => {
    ...
    expect(canonicalThemePreference("swarm-code-chat-dark")).toBe("swarm-code-chat-dark");
  ...
  it("recognizes only preferences the runtime can render", () => {
    for (const preference of ["light", "dark", "system", SWARM_CODE_CHAT_THEME.id, GROVE_THEME.id]) {
...
describe("singleAppearanceOf", () => {
  it("reports the only half a theme can claim, and null for a pair", () => {
    const { variants: _pair, ...base } = SWARM_CODE_CHAT_THEME;
    expect(singleAppearanceOf({ ...base, id: "x", appearance: "dark" })).toBe("dark");
    expect(singleAppearanceOf(SWARM_CODE_CHAT_THEME)).toBe(null);
  });
});
