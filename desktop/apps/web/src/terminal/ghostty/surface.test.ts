...
describe("terminalLinkAtPositionWithRange", () => {
  it("maps terminal cells to UTF-16 offsets after a wide emoji", () => {
    const cells = [
      cell("🙂"),
      cell(""),
      ...Array.from("https://swarmcode.dev", (character) => cell(character)),
    ];
    ...
    expect(terminalLinkAtPositionWithRange([row], 0, 2)?.text).toBe("https://swarmcode.dev");
  ...
  it("refuses links truncated at the viewport edges instead of mis-resolving", () => {
    ...
    const tailCut = [row("https://swarmcode.dev", false, true)];
    expect(terminalLinkAtPositionWithRange(tailCut, 0, 8)).toBeNull();
    // A partial bottom row is provably complete and still resolves.
    const complete = [row("https://swarmcode.dev", false), row("", false)];
    expect(terminalLinkAtPositionWithRange(complete, 0, 8)?.text).toBe("https://swarmcode.dev");
    ...
    const unwrittenTail: GhosttyRow = {
      ...
      text: "https://swarmcode.dev",
      isWrapContinuation: false,
      wrapsToNext: false,
    };
    expect(terminalLinkAtPositionWithRange([unwrittenTail], 0, 8)?.text).toBe("https://swarmcode.dev");
  });
});
