...
import { EnvironmentRegistry } from "@swarmcode/client-runtime/connection";
import { request, runStream } from "@swarmcode/client-runtime/rpc";
import { makeEnvironmentHttpApiClient } from "@swarmcode/client-runtime/rpc";
import { ManagedRelay, relayProtectedErrorMessage } from "@swarmcode/client-runtime/relay";
  ...
  return Effect.gen(function* () {
    ...
    if (status.status === "unsupported") {
      return yield* new CloudEnvironmentLinkError({
        message: `Swarm Code cannot install the relay client automatically on ${status.platform}-${status.arch}.`,
      });
    }
      ...
      return yield* new CloudEnvironmentLinkError({
        message:
          installedStatus.status === "unsupported"
            ? `Swarm Code cannot install the relay client automatically on ${installedStatus.platform}-${installedStatus.arch}.`
...
// independently of Swarm Connect while clients reach the environment out of band.
...
> {
  return Effect.gen(function* () {
    ...
    if (!configuredRelayUrl) {
      return yield* new CloudEnvironmentLinkError({
        message: "SWARMCODE_RELAY_URL is not configured.",
      });
    }
