import { relayClerkTokenOptions } from "@swarmcode/shared/relayAuth";
...
export class CloudPublicConfigMissingError extends Schema.TaggedError<CloudPublicConfigMissingError>()(
  ...
  {
    key: Schema.Literal("SWARMCODE_CLERK_JWT_TEMPLATE"),
  },
) {
...
export function resolveCloudPublicConfig(): CloudPublicConfig {
  return {
    ...
    relayUrl: normalizeSecureRelayUrl(
      (import.meta.env.VITE_SWARMCODE_RELAY_URL as string | undefined) ?? "",
    ),
    relayTracing: {
...
export function resolveRelayClerkTokenOptions() {
  ...
  if (!clerkJwtTemplate) {
    throw new CloudPublicConfigMissingError({ key: "SWARMCODE_CLERK_JWT_TEMPLATE" });
  }
