...
import {
  ...
  SWARM_CODE_LIGHT_THEME_COLORS,
  SWARM_CODE_DARK_THEME_COLORS,
  RESERVED_THEME_IDS,
  THEME_COLOR_ROLES,
...
export const SWARM_CODE_CHAT_THEME_ID = "swarm-code-chat" as const;
export const T3_CHAT_THEME_ID = SWARM_CODE_CHAT_THEME_ID;
...
const LEGACY_SWARM_CODE_CHAT_DARK_THEME_ID = "swarm-code-chat-dark";
...
const LEGACY_THEME_ID_ALIASES: Readonly<Record<string, string>> = {
  [LEGACY_SWARM_CODE_CHAT_DARK_THEME_ID]: SWARM_CODE_CHAT_THEME_ID,
...
export function canonicalThemePreference(theme: string): string {
  return theme === LEGACY_SWARM_CODE_CHAT_DARK_THEME_ID ? theme : normalizeThemeId(theme);
}
...
function legacyThemeMode(theme: ThemePreference): ThemeAppearance | null {
  return theme === LEGACY_SWARM_CODE_CHAT_DARK_THEME_ID ? "dark" : null;
}

/**
 * The standard Swarm Code look as a theme palette, for seeding a new theme when
...
export function getStandardThemeColors(appearance: ThemeAppearance): ThemeColors {
  if (appearance === "dark") {
    return (standardDarkThemeColors ??= decodeThemeColors(SWARM_CODE_DARK_THEME_COLORS));
  }
  return (standardLightThemeColors ??= decodeThemeColors(SWARM_CODE_LIGHT_THEME_COLORS));
}
...
/**
 * The status colors Swarm Code shows without a theme, read from the app's own
...
export function getDefaultThemeColors(appearance: ThemeAppearance): ThemeColors {
  return appearance === "dark" ? SWARM_CODE_CHAT_THEME.variants!.dark! : SWARM_CODE_CHAT_THEME.colors;
}
