...
public final class MCPLoopbackTransport: MCPTransport {
 ...
 // Guarded by `transportQueue` / @MainActor coordinator.
 ...
 // Loopback transport is primarily server-side; outgoing requests are
 // routed to the matching plugin connection if one exists, otherwise
 // we return an error indicating no route is available.
 ...
 // Fire-and-forget to all registered plugin connections (best-effort).
 ...
 // Convert incoming JSON-RPC request into a stream event.
 ...
 ///
 /// Plugins should call this (or the framework should call it on behalf of
 /// a known plugin) before the plugin is expected to send requests.
 ...
 ptr.withMemoryRebound(to: sockaddr.self, capacity: 1) {
 Darwin.getpeername(fd, $0, &boundLen)
 }
 }
 ...
 guard buffer.count >= bodyStart.utf8Offset + contentLength else {
 return nil // Incomplete body — wait for more data.
 }

 let body = buffer.subdata(in: bodyStart ..< bodyStart.utf8Offset + contentLength)
 buffer.removeSubrange(buffer.startIndex ..< bodyStart.utf8Offset + contentLength)
 ...
 // Convention: plugin id encoded in the method namespace, e.g.
 // "plugin.blender/tools.list".
 let components = request.method.split(separator: "/", maxSplits: 1)
 if components.count >= 2, let namespace = components.first {
 if namespace != "plugin" && namespace != "mcp" {
...
private extension Data {
 var utf8Offset: Int { self.startIndex.utf8Offset(in: self) }
}

extension String.Index {
 fileprivate func utf8Offset(in string: String) -> Int {
 string.utf8.distance(from: string.utf8.startIndex, to: self)
 }
}
...
private struct BSDStream: Stream, Sendable {
 ...
 throw MCPTransportError.ioError("write() error: \(err)")
 }
