...
public struct BatchedChatTurnEngine: ChatTurnEngine {
 ...
 let context = await buildContext(session: session, newMessage: message)
 ...
 for try await event in await llmProvider.stream(context: context, tools: tools) {
 ...
 for try await finalEvent in await llmProvider.stream(context: context + [toolMessage], tools: tools) {
 ...
 for try await event in await llmProvider.stream(context: context, tools: []) {
 continuation.yield(event)
 if case .done = event {
 continuation.finish()
 return
...
public enum MessageRole: String, Codable, Equatable, CaseIterable {
 case system
 case user
 case assistant
 case tool
...
public struct ChatStreamParser {
 ...
 events.append(.chunk(parsed.content))
 }
 }
 }
